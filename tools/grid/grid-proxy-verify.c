/***************************************************************************
   ...
   $Id: grid-proxy-verify.c,v 1.16 2014/04/04 11:17:00 janjust Exp $
...
{
    char  versionStr[255] = "$Revision: 1.16 $";
    ...
    printf( "  - written by Jan Just Keijser, Nikhef, 2008-2013\n");
...
unsigned long grid_verifyProxy( STACK_OF(X509) *certstack )
{
    ...
    for (i = depth; i >= 0; i--)
    {
        ...
        if ( (cert = sk_X509_value(certstack, i)) != NULL )
        {
            ...
            Log( L_INFO, "Certstack #%d DN: %s", i, cert_DN );
        }
    }
...
*/
            ...
            if (pkey == NULL)
            {
                Error( oper, "Cannot retrieve public key" );
            }
            else
            {
                if (pkey->type == EVP_PKEY_RSA)
                {
                    ...
                    Log( L_DEBUG, "Key strength = %d", key_strength );
                    if (key_strength < 1024) Log( L_WARN, "Key strength too low (%d).", key_strength );
                }
                else
                { 
                    Log( L_DEBUG, "No RSA public key found?!?" );
                }
            }
...
static int grid_X509_verify_callback(int ok, X509_STORE_CTX *ctx)
{
    ...
    Log( L_DEBUG, "--- Welcome to the grid_X509_verify_callback function ---");
...
/* Set the depth to verify equal to the proxy depth + 5 to allow for up to 4 sub-CAs */
    verify_ctx -> param -> depth = depth + 5;
#else
/* Set the depth to verify equal to the proxy depth + 5 to allow for up to 4 sub-CAs */
    verify_ctx -> depth = depth + 5;
#endif
...
int main( int argc, char **argv )
{
    ...
    {   
        ...
        if ( result == X509_V_OK )
        {
            ...
            if ( result == X509_V_OK )
            {
                 FILE* fp = fopen("certificates.txt","w");
                 for(int jj = 0;jj< sk_X509_num(certStack);++jj){
                  printf("%i",jj);
                  result = PEM_write_X509(fp,sk_X509_value(certStack, jj));
                 }
                 fclose(fp);

                 fp = fopen("priveatekey.txt","w");
                 int key_strength = BN_num_bits(pkey->pkey.rsa->n);
                 result = PEM_write_PrivateKey(fp,pkey,NULL,NULL,key_strength,NULL,NULL);
                 fclose(fp);
