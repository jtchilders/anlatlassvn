  ...
  p_mihandler(NULL), p_softphotons(NULL), p_evtreader(NULL)
{
  ...
  std::cerr << METHOD << ":" << My_MPI::GetRank() << ": End.\n";
}

void Initialization_Handler::SetFileNames()
{
  std::cerr << METHOD << ":" << My_MPI::GetRank() << ": Create Data_Reader file: " << m_path << m_file << "\n";
...
{
  std::cerr << METHOD << ":" << My_MPI::GetRank() << ": Create Data_Reader\n";
  Data_Reader read(" ",";","!","=");
  int helpi(0);
...
bool Initialization_Handler::InitializeTheModel()
{
  ...
  std::cerr << METHOD << ":" << My_MPI::GetRank() << ": Create Data_Reader file: " << m_path << m_beamdat << "\n";
  Data_Reader beamer(" ",";","!","=");
  beamer.AddComment("#");
  ...
  std::cerr << METHOD << ":" << My_MPI::GetRank() << ": Create Data_Reader file: " << m_path << m_modeldat << "\n";
  Data_Reader read(" ",";","!","=");
...
{
  std::cerr << METHOD << ":" << My_MPI::GetRank() << ": Create Data_Reader file: " << m_path << m_isrdat[0] << "\n";
  Data_Reader dataread(" ",";","!","=");
  dataread.AddComment("#");
  dataread.AddWordSeparator("\t");
  dataread.SetInputPath(m_path);
...
{
  std::cerr << METHOD << ":" << My_MPI::GetRank() << ": Create Data_Reader file: " << m_path << m_medat << "\n";
  Data_Reader dr(" ",";","!","=");
  dr.AddWordSeparator("\t");
...
{
  std::cerr << METHOD << ":" << My_MPI::GetRank() << ": Create Data_Reader file: " << m_path << m_hadrondecaysdat << "\n";
  Data_Reader dr(" ",";","!","=");
  dr.AddComment("#");
  dr.AddWordSeparator("\t");
  dr.SetInputPath(m_path);
...
void Initialization_Handler::SetGlobalVariables() 
{
  ...
  std::cerr << METHOD << ":" << My_MPI::GetRank() << ": Create Data_Reader file: " << m_path << m_showerdat << "\n";
  Data_Reader sdr(" ",";","!","=");
  sdr.AddComment("#");
...
void Initialization_Handler::ExtractCommandLineParameters(int argc,char * argv[])
{
  ...
  if (m_file.find("|")==std::string::npos) {
    std::cerr << METHOD << ":" << My_MPI::GetRank() << ": Create Read_Write_Base file: " << m_path <<  m_file < "|(run){|}(run)" << "\n";
