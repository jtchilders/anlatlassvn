...
void Read_Write_Base::SplitInFileName(const size_t &i)
{
  ...
  std::cerr <<METHOD<<": Set '"<<m_filebegin.back()
		 <<"'->'"<<m_fileend.back()<<"'.\n"; 
}
...
{
  std::cerr << METHOD << ":" << My_MPI::GetRank() << ": Start: file = " << InputPath(i) << " " << InputFile(i) << ", i = " << i << " mode = " << mode << "\n";
  if (InputPath(i)+InputFile(i)==nullstring) {
    std::cerr << METHOD << ":" << My_MPI::GetRank() << ": if(nullstring)\n";
    if (m_addcommandline && CommandLine().size()>0) {
    std::cerr << METHOD << ":" << My_MPI::GetRank() << ": if(nullstring) then if(m_addcommandline)\n";
  ...
  std::cerr << METHOD << ":" << My_MPI::GetRank() << ": after if(nullstring), InFileMode(i) = " << InFileMode(i) << "\n";
  if (InFileMode(i)==fom::unknown) SetInFileMode(fom::permanent);
  std::cerr << METHOD << ":" << My_MPI::GetRank() << ": m_filecontent[i].empty() = " << m_filecontent[i].empty() << "\n";
  if (!m_filecontent[i].empty()&&mode==0) return true;
  SplitInFileName(i); // this looks at InputFile(i) for things like "Run.dat|{...}" and sets InputFile(i) = "Run.dat"
  std::string lastline, file(InputPath(i)+InputFile(i));
  file+="|"; // Adding back what we removed!?
  ...
  std::cerr << METHOD << ":" << My_MPI::GetRank() << ": file = " << file << "\n";
  bool inbuf(s_buffermap.find(file)!=s_buffermap.end());
  std::cerr << METHOD << ":" << My_MPI::GetRank() << ": inbuf = " << inbuf << "\n";
  String_Vector &cbuffer(s_buffermap[file]);
  std::cerr <<METHOD<<":" << My_MPI::GetRank() << ": ("<<this<<") checks buffer '"
		   <<file<<"' -> "<<inbuf<<"("<<&cbuffer<<")\n";
  if (inbuf) {
  ...
  std::cerr << METHOD << ":" << My_MPI::GetRank() << ": buffer.empty() = " << buffer.empty() << "\n";
  if (mode&2 || buffer.empty()) { // read in file contents to buffer 
    std::cerr <<METHOD<<":" << My_MPI::GetRank() << ": ("<<this<<") reads '"<<file
		   <<"', mode = "<<infile.Mode()<<"\n";
    infile.Open();
    buffer.clear();
    if (*infile) {
  ...
  } // end if(mode&2 || buffer.empty())
  std::cerr << METHOD << ":" << My_MPI::GetRank() << ": After if(inbuf), buffer.empty() = " << buffer.empty() << "\n";
  m_filecontent[i].clear(); // reset file content
  bool checkbegin=(bool)(m_filebegin.size()>0);
  bool checkend=(bool)(m_fileend.size()>0);
  int filebegin=0;
  unsigned int occurrence=0;
  if (!buffer.empty()) { // buffer should now be full
	...
	if (filebegin==0) {
	  ...
	  } // end for(m_fileend.size())
	} // end if(checkend)
  // if line is between "(run){" and "}(run)" add to cbuffer
	if (lastline.length()>0) cbuffer.push_back(lastline);
      }// end if(checkbegin)
      else if (lastline.length()>0) { // if no (run){|}(run) type stuff was included in the filename, then just copy the entire file to buffer
	if (lastline.length()>0) cbuffer.push_back(lastline);
      }
    } // end for(buffer.size())
    // now we have all the content from the file between our "(run)"
    std::cerr << METHOD << ":" << My_MPI::GetRank() << ": cbuffer before InterpreteBuffer\n";
    for(unsigned int iii=0;iii<cbuffer.size();++iii)
      std::cerr << METHOD << ":" << My_MPI::GetRank() << ": cbuffer[" << iii << "] = " << cbuffer[iii] << "\n";
    InterpreteBuffer(cbuffer);
    std::cerr << METHOD << ":" << My_MPI::GetRank() << ": cbuffer after InterpreteBuffer\n";
    for(unsigned int iii=0;iii<cbuffer.size();++iii)
      std::cerr << METHOD << ":" << My_MPI::GetRank() << ": cbuffer[" << iii << "] = " << cbuffer[iii] << "\n";
    for (size_t j(0);j<cbuffer.size();++j)
      AddFileContent(cbuffer[j],i);
    std::cerr << METHOD << ":" << My_MPI::GetRank() << ": m_filecontent after AddFileContent\n";
    for(unsigned int iii=0;iii<m_filecontent.size();++iii)
    for(unsigned int jjj=0;jjj<m_filecontent[iii].size();++jjj){
      std::cerr << METHOD << ":" << My_MPI::GetRank() << ": m_filecontent[" << iii << "][" << jjj << "] = ";
    for(unsigned int kkk=0;kkk<m_filecontent[iii][jjj].size();++kkk){
      std::cerr << m_filecontent[iii][jjj][kkk];
    std::cerr << " = ";
    }
    std::cerr << "\n";
    }
  } // end if(!buffer.empty())
  std::cerr << METHOD << ":" << My_MPI::GetRank() << ": after if(!buffer.empty())\n";
  } // end else(if(inbuf))
  std::cerr << METHOD << ":" << My_MPI::GetRank() << ": after if(inbuf)-else, m_addcommandline = " << m_addcommandline << " CommandLine().size() = " << CommandLine().size() << "\n";
  if (m_addcommandline && CommandLine().size()>0) {
    for (size_t j(0);j<CommandLine().size();++j)
      AddFileContent(CommandLine()[j],i);
  }
  std::cerr <<METHOD<<":" << My_MPI::GetRank() << ": Read file content '"<<InputPath()
		 <<InputFile()<<"' {\n";
  for (size_t j(0);j<m_filecontent[i].size();++j) {
    std::cerr<<"  ";
    for (size_t k(0);k<m_filecontent[i][j].size();++k)
      std::cerr<<"'"<<m_filecontent[i][j][k]<<"' ";
    std::cerr<<"\n";
  }
  std::cerr<<"}\n";
  std::cerr << METHOD << ":" << My_MPI::GetRank() << ": m_filecontent[i].size() = " << m_filecontent[i].size() << "\n";
