...
#ifdef USING__MPI
  MPI::Init(argc,argv);
  std::cout << "MPI=-=" << MPI::COMM_WORLD.Get_rank() << "=-=" << __PRETTY_FUNCTION__ << "=-=Begin\n";
#endif
  Sherpa *Generator(new Sherpa());
#ifdef USING__MPI
  My_MPI::PrintMessage(__PRETTY_FUNCTION__,"InitializeTheRun");
#endif
  try {
    ...
    if (nevt>0) {
#ifdef USING__MPI
      My_MPI::PrintMessage(__PRETTY_FUNCTION__,"InitializeTheEventHandler");
#endif
...
#ifdef USING__MPI
      My_MPI::PrintMessage(__PRETTY_FUNCTION__,"for(rpa->gen.NumberOfEvents)");
#endif
...
#ifdef USING__MPI
  My_MPI::PrintMessage(__PRETTY_FUNCTION__,"delete Generator");
#endif
  delete Generator;
#ifdef USING__MPI
  MPI::Finalize();
  My_MPI::PrintMessage(__PRETTY_FUNCTION__,"End");
#endif
  return 0;
}
