...
{ 
  ...
  for (size_t i=0;i<svv.size();++i) {
    if (svv[i].size()==1) m_level=ATOOLS::ToType<int>(svv[i][0]);
      ...
      if (lev&64) m_contextmpidebugging.insert(svv[i][0]);
    }
    else {
...
std::ostream &Message::MPIDebugging() const
{
  if (m_level & 64) return *p_output;
  return *p_no;
}
...
bool Message::LevelIsMPIDebugging(const std::string& context) const
{
  for (std::set<std::string>::reverse_iterator rit=m_contextmpidebugging.rbegin();
       rit!=m_contextmpidebugging.rend(); ++rit) {
    if (context.find(*rit)!=std::string::npos) return true;
  }
  return false;
}
