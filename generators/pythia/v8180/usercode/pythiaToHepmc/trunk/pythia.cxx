...
void broadcast_line(char* line,const std::string rank,std::vector<std::string>& commands);
void receive_commands(const std::string rank,std::vector<std::string>& commands);
...
int main(int argc,char** argv){
   ...
   if(world_rank == 0){
      ...
      while(!file.eof()){
         file.getline(line,COMMAND_SIZE);
         if(file.eof()) continue;
         broadcast_line(line,str_world_rank,commands);
         std::cout << line << "\n";
      }
      file.close();
      // send final message to all ranks that commands are done
      char msg[sizeof(FINISHED_MSG)] = FINISHED_MSG;
      int retval = MPI_Bcast(msg,sizeof(FINISHED_MSG),MPI_CHAR,0,MPI_COMM_WORLD);
      if(retval != MPI_SUCCESS){
         std::cout << "ERROR in RANK " << str_world_rank << " sending FINISHED_MSG ";
         return -1;
      }
      std::cout << "======================================================\n";
   }
   else{
      receive_commands(str_world_rank,commands);
   }
...
void broadcast_line(char* line,const std::string rank,std::vector<std::string>& commands){
   ...
   int return_value = 0;
   int ch = int(line[0]);
   // Only send commands that begin with numbers or letters
   if(  
         ( 48 <= ch && ch <= 57 ) // 0 - 9
      || ( 65 <= ch && ch <= 90 ) // A - Z
      || ( 97 <= ch && ch <= 122) // a - z
     )
   {
      ...
      return_value = MPI_Bcast(line,COMMAND_SIZE,MPI_CHAR,0,MPI_COMM_WORLD);
      if(return_value != MPI_SUCCESS){
         std::cout << "ERROR broadcasting or receiving in RANK " << rank << "\n";
      }
      std::cout << " RANK " << rank << " command: " << ss.str() << "\n";
   }
}
...
void receive_commands(const std::string rank,std::vector<std::string>& commands){
   ...
   while(strcmp(command,FINISHED_MSG) != 0){
      return_value = MPI_Bcast(command,COMMAND_SIZE,MPI_CHAR,0,MPI_COMM_WORLD);
      if(return_value != MPI_SUCCESS){
         std::cout << "ERROR broadcasting or receiving in RANK " << rank << "\n";
      }
      if(strcmp(command,FINISHED_MSG) == 0) break;
      std::stringstream ss;
      ss << command;
      commands.push_back(ss.str());
      std::cout << " RANK " << rank << " command: " << ss.str() << "\n";
