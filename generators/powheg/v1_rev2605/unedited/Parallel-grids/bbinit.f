...
c 1 for starting multigrid operation (many processes)
c 0 for storing xgrid in multigrid operation (1 process)
c 2,3,4 ... for refining (many processes)
      flg_multigrid = powheginput('#multigrid')
...
c      if(ncall1*itmx1.eq.0) then
c         write(*,*) ' ncall1 set to 0; nothing else to do'
c         call write_counters
c         call exit(0)
c      endif
      iret1=0
      iret2=0
     ...
     2     ifold,ifoldrm)
      if(iret1.ne.0) then
         if(flg_multigrid.ne.1.and.flg_multigrid.ne.0) then
            call loadxgrid(iret2,xgrid,xint,xgridrm,xintrm)
         else
            iret2 = -1
         endif
      endif
      if(iret2.ne.0.or.flg_multigrid.gt.0) then
         write(*,*)
...
c If multigrid is set, set a different random number also for grids
         if(flg_multigrid.gt.0)
     1     call setrandom(rnd_initialseed+flg_multigrid-1,rnd_i1,rnd_i2)
         call newunit(iun)
         storelabel='btl'
     ...
     #         iun,xgridrm,xintrm,ymaxrm,sigrm,errrm)
         ...
         if(.not. flg_multigrid.gt.0) then
