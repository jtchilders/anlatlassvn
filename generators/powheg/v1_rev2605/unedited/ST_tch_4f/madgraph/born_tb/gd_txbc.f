...
C  
C FOR PROCESS : g d -> t~ b c  
C  
C Crossing   1 is g d -> t~ b c  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C  
      ...
      common/to_amps_gd_txbc/  amp2,       jamp2
...
C ----------
     ...
     &             NHEL(JJ,IHEL).EQ.INT(SIGN(1d0,POL(JJ)))) THEN
      ...
      REAL*8 FUNCTION gd_txbc(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
     ...
     &     9   ))                                                          
      CALL IOVXXX(W(1,2   ),W(1,5   ),W(1,9   ),GWFCD ,AMP(2   ))          
      JAMP(   1) = +AMP(   1)+AMP(   2)
      gd_txbc = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          ...
          ENDDO
          gd_txbc =gd_txbc+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
      Do I = 1, NGRAPHS
