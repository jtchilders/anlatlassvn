...
C  
C FOR PROCESS : g c~ -> t~ b d~  
C  
C Crossing   1 is g c~ -> t~ b d~  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C  
      ...
      common/to_amps_gcx_txbdx/  amp2,       jamp2
...
C ----------
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=gcx_txbdx(P ,NHEL(1,IHEL),JC(1))            
               DO JJ=1,nincoming
     ...
     &             NHEL(JJ,IHEL).EQ.INT(SIGN(1d0,POL(JJ)))) THEN
      ...
      REAL*8 FUNCTION gcx_txbdx(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
     ...
     &     9   ))                                                          
      CALL IOVXXX(W(1,5   ),W(1,2   ),W(1,9   ),GWFCD ,AMP(2   ))          
      JAMP(   1) = -AMP(   1)-AMP(   2)
      gcx_txbdx = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          ...
          ENDDO
          gcx_txbdx =gcx_txbdx+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
      Do I = 1, NGRAPHS
