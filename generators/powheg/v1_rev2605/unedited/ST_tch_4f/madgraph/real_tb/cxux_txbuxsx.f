      SUBROUTINE Scxux_txbuxsx(P1,ANS)
C  
...
C  
C FOR PROCESS : c~ u~ -> t~ b u~ s~  
C  
C Crossing   1 is c~ u~ -> t~ b u~ s~  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C  
      ...
      common/to_amps_cxux_txbuxsx/  amp2,       jamp2
...
C ----------
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=cxux_txbuxsx(P ,NHEL(1,IHEL),JC(1))            
               DO JJ=1,nincoming
     ...
     &             NHEL(JJ,IHEL).EQ.INT(SIGN(1d0,POL(JJ)))) THEN
      ...
      REAL*8 FUNCTION cxux_txbuxsx(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
     ...
     &     11  ))                                                          
      ...
      DO I = 1, NCOLOR
          ...
          ENDDO
          cxux_txbuxsx =cxux_txbuxsx+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
      Do I = 1, NGRAPHS
