...
c            if(rad_kinreg.eq.1) write(66,*) 1,xnorm
c            if(rad_kinreg.eq.2) write(67,*) 1,xnorm
            if (debug) then
               if(xnorm.gt.10.) then
                  write(*,*) ' **********************'
                  write(*,*) 
     ...
     $              ,'  jac: ',kn_jacborn 
               call pdfcall(1,kn_xb1,pdf1)
               call pdfcall(2,kn_xb2,pdf2)
               write(*,*) " born PDF's ",pdf1(flst_born(1
     $              ,rad_ubornidx)),pdf2(flst_born(2,rad_ubornidx))
                  call printevent
c$$$                  do k=1,10000
c$$$                     write(*,*) ' enter csitilde,y'
c$$$                     read(*,*) kn_csitilde, kn_y
c$$$                     call gen_real_phsp_isr_rad0
c$$$                     ptsq=pwhg_pt2()
c$$$                     if(ptsq.gt.rad_ptsqmin) then
c$$$                        call set_rad_scales(ptsq)
c$$$                        call sigreal_rad(sig)
c$$$                        sig=sig*kn_jacreal
c$$$                        xnorm=sig/born/pwhg_upperb_rad()
c$$$                        write(*,*) 'xnorm:',
c$$$     $                       xnorm
c$$$                        call printevent
c$$$                     endif
...
c$$$               if(xnorm.gt.3e-5.and.xnorm.lt.3.1e-5) then
c$$$                  write(*,*) ' **********************'
c$$$                  write(*,*) ' typical event:',
c$$$     $                 xnorm
c$$$                  call printevent
c$$$               endif
            endif
...
c
     ...
     # (flst_born(j,rad_ubornidx),j=1,nlegborn)
      ...
      write(*,*) 'real kin'
      do k=1,nlegreal
         write(*,100) (kn_cmpreal(mu,k),mu=0,3)
      enddo
      write(*,*) ' real flavour:'
      do k=1,flst_born2alr(0,rad_ubornidx)
         alr=flst_born2alr(k,rad_ubornidx)
         write(*,*) (flst_alr(j,alr),j=1,nlegreal)
         call setreal(kn_cmpreal,flst_alr(1,alr),amp2real)
         write(*,*) 'amp2real = ',amp2real
      enddo
      write(*,*) ' ptb (real) ',
     $     sqrt(kn_cmpreal(1,4)**2+kn_cmpreal(2,4)**2)
      write(*,*) ' sb5 (real) ',
     $     dotp(kn_cmpreal(0,4),kn_cmpreal(0,5))
      write(*,*) ' sb6 (real) ',
     $     dotp(kn_cmpreal(0,4),kn_cmpreal(0,6))

c     compute cos of the angle between b quark and light partons
      E6 = kn_cmpreal(0,6)
      E5 = kn_cmpreal(0,5)
      Eb = kn_cmpreal(0,4)
      betab=
     $sqrt(kn_cmpreal(1,4)**2+kn_cmpreal(2,4)**2+kn_cmpreal(3,4)**2)/Eb
      yb5 = 1-dotp(kn_cmpreal(0,4),kn_cmpreal(0,5))/E5/Eb !=betab*cos
      yb5=yb5/betab
      yb6 = 1-dotp(kn_cmpreal(0,4),kn_cmpreal(0,6))/E6/Eb !=betab*cos
      yb6=yb6/betab
      write(*,*) 'yb5 ',yb5
      write(*,*) 'yb6 ',yb6
