...
double qqttg2virtfin(const FourMomentum & p1,const FourMomentum & p2,
  ...
  static ErrorCounter LargeCorrection_qq("Large correction in qqttg2virtfin drop event.");
  ...
  if ( !initialized ) {
    ...
    cout << "# -------------------------------------------------------------"
	 << endl;
    cout << "# qqttg2virtfin: Initialize color matrix..."; 
    LOcolorqq(CLO);
    initialized = true;
    cout << " done." << endl;
    cout << "# -------------------------------------------------------------"
  ...
  ////// REMOVED iopqq(Mij,p1,p2,p3,kq,kqb);
  /*
  ...
  if (  CheckLargeCorrections==true  ){
    ...
    LargeCorrection_qq.addError();
    char errmess[]="Large correction in Vfin_qq: event dropped";
    increasecnt_(errmess,strlen(errmess));
    amp2 = 0;
    // MODIFIED: instead of 0 returns a quiet NaN
    // that  is catched by POWHEG
