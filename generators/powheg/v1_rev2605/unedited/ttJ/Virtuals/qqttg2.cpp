// $Modified: Tue Jul 31 15:29:36 2007 by uwer $
...
void iopqq(complex<double> Mij[MAXCOLOR][MAXCOLOR],
  ...
  for(int c1=1; c1 < MAXCOLOR; c1++) {
	...
	* (
	   ...
	   * ( Vq( 2.0*dotp(ktb,p3),m,0.) - (pi2/3.0) 
	   ...
	   + cg3t[c1][c2] 
	   * ( Vg( 2.0*dotp(p3,kt), 0., m, mf, kappa) - (pi2/3.0) 
	       + ( Gamma_g(mf) + gammag*log(mu2/2.0/dotp(p3,kt)) 
		   + gammag + Kg )/CA 
	       )
	   
	   + cg3tb[c1][c2] 
	   * ( Vg( 2.0*dotp(p3,ktb), 0., m, mf, kappa) - (pi2/3.0) 
	       + ( Gamma_g(mf) + gammag*log(mu2/2.0/dotp(p3,ktb)) 
		   + gammag + Kg )/CA 
	       )
...
double Ioperator_qq(const FourMomentum & p1, const FourMomentum & p2, 
  ...
  EvalLOfqq(f, p1,p2,p3,kq,kqb);

  iopqq(Mij,p1,p2,p3,kq,kqb);

  /*
   *  Sum over the polarization and square it:
   */
  EvalSMEqq(S,p1, p2, p3,q1,q2,r1,r2);

  for (int tspin=0; tspin < 2; tspin++) {

    if (tspin == 1) 
      EvalSMEqq(S,p1, p2, p3,q1,q2,r2,r1);

    if (tspin == 2) 
      EvalSMEqq(S,p1, p2, p3,q2,q1,r1,r2);

    if (tspin == 3) 
      EvalSMEqq(S,p1, p2, p3,q2,q1,r2,r1);
...
double qqttg2virt(const FourMomentum & p1,const FourMomentum & p2,
  ...
  if ( !initialized ) {
    ...
    cout << "# -------------------------------------------------------------"
	 << endl;
    cout << "# qqttg2virt: Initialize color matrix..."; 
    LOcolorqq(CLO);
    initialized = true;
    cout << " done." << endl;
    cout << "# -------------------------------------------------------------"
   ...
   */
  try {
    pentagon_qq(fivepoint,p1,p2,p3,kq,kqb, 0.0); 
  ...
  evaluatePVcoefficients(PVcoeffs,p1,p2,p3,kq,kqb,Bptr,Cptr,Dptr);

  MasslessFermionLoops_qq(f_nfl,p1,p2,p3,kq,kqb,Bptr,Cptr,Dptr,1.0 );   

  TopLoops_qq(f_top,p1,p2,p3,kq,kqb,Bptr,Cptr,Dptr, 1.0); 

  RemainingDiags_qq(rest,p1,p2,p3,kq,kqb,Bptr,Cptr,Dptr, 1.0);

  Ghosts_qq(ghosts,p1,p2,p3,kq,kqb,Bptr,Cptr,Dptr, 1.0);

  masscounterterm_qq(mct,p1,p2,p3,kq,kqb, 1.0);

  EvalLOfqq(flo, p1, p2, p3, kq, kqb);
...
#ifdef TESTBADPOINTS
   static EPSHisto epsqq("EPS_large_qq.top");
   epsqq.fill(log(fabs(0.01*amp2/amplo2)));
#endif
   if ( fabs(0.01*amp2/amplo2) > 100. ) {
    cout << "# Large correction drop event \n";
    cout << "# born = " << amplo2 << " nlo_qq = " << amp2*0.01 << endl;
    amp2 = 0.0;
  }
  // 2 (spin) * 2 (2Re())
  return(4.0*amp2);
