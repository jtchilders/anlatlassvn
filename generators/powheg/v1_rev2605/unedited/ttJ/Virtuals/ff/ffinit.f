C$Modified:   Tue Nov 9 12:46:16 EST 2010 by S. Alioli  $
	subroutine ffchangeflags
***#[*comment:***********************************************************
*	change the value of flags after the call to ffinit		*
***#]*comment:***********************************************************
	include 'ff.h'
	ltest=.true.
	end
...
*
**** Changed 22.09.11 by SA: the following operation produces a FPE (underflow)
**** on ifort compiler (no -no-ftz or -ftz option avoid this behavior)
	goto 6
**** so skip and set by hand
	s = 1
	xalogm = 1
	do 5 i=1,10000
	    s = s/2
...
******** The above comparison doesn't work, set xclogm by hand:  
	xalogm = 1d-300
********
