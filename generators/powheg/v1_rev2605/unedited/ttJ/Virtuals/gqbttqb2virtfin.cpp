...
double gqbttqb2virtfin(const FourMomentum & p1,const FourMomentum & p2,
  ...
  complex<double> tmp;
  static ErrorCounter LargeCorrection_gqb("Large correction in gqbttqb2virtfin drop event.");
  ...
  if ( !initialized ) {
    ...
    cout << "# -------------------------------------------------------------"
	 << endl;
    cout << "# gqbttqb2virtfin: Initialize color matrix..."; 
    LOcolorqq(CLO);
    initialized = true;
    cout << " done." << endl;
    cout << "# -------------------------------------------------------------"
  ...
  //// REMOVED iopgqb(Mij,p1,p2,p3,kq,kqb);
  /*
  ...
  if (  CheckLargeCorrections==true  ){
    ...
    LargeCorrection_gqb.addError();
     char errmess[]="Large correction in Vfin_gqb: event dropped";
    increasecnt_(errmess,strlen(errmess));
    amp2 = 0;
    // MODIFIED: instead of 0 returns a quiet NaN
    // that  is catched by POWHEG
