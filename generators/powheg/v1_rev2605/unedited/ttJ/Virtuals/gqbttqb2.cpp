// $Modified: Tue Jul 31 15:29:49 2007 by uwer $
...
void iopgqb(complex<double> Mij[MAXCOLOR][MAXCOLOR],
  ...
  for(int c1=1; c1 < MAXCOLOR; c1++) {
	...
	* (
	   ...
	   * ( Vq( 2.0*dotp(p3,p2), 0., 0.) - (pi2/3.0) 
	    ...
	    */
	   + cqbt[c1][c2] 
	   * ( Vq( 2.0*dotp(p2,kt), 0., m) - (pi2/3.0) 
	       + ( Gamma_q() + gammaq*log(mu2/2.0/dotp(p2,kt)) 
		   + gammaq + Kq )/CF 
	       )
	   
	   + cqbtb[c1][c2] 
	   * ( Vq( 2.0*dotp(p2,ktb), 0., m) - (pi2/3.0) 
	       + ( Gamma_q() + gammaq*log(mu2/2.0/dotp(p2,ktb)) 
		   + gammaq + Kq )/CF 
	       )
	   + cqqb[c1][c2] 
	   * ( Vq( 2.0*dotp(p2,p3), 0., 0.) - (pi2/3.0) 
	       + ( Gamma_q() + gammaq*log(mu2/2.0/dotp(p2,p3)) 
		   + gammaq + Kq )/CF 
	       )
	   // initial -- final I_b 
	   /*
	   ...
	   * ( Vq( 2.0*dotp(p3,p1), 0., 0.) - (pi2/3.0) 
	    ...
	    */
	   + cg3t[c1][c2] 
	   * ( Vg( 2.0*dotp(p1,kt), 0., m,mf, kappa) - (pi2/3.0) 
	   + ( Gamma_g(mf) + gammag*log(mu2/2.0/dotp(p1,kt)) 
	       + gammag + Kg )/CA 
	       )
	   
	   + cg3tb[c1][c2] 
	   * ( Vg( 2.0*dotp(p1,ktb), 0., m,mf, kappa) - (pi2/3.0) 
	       + ( Gamma_g(mf) + gammag*log(mu2/2.0/dotp(p1,ktb)) 
		   + gammag + Kg )/CA 
	       )
	   + cqg3[c1][c2] 
	   * ( Vg( 2.0*dotp(p1,p3), 0., 0.,mf, kappa) - (pi2/3.0) 
	       + ( Gamma_g(mf) + gammag*log(mu2/2.0/dotp(p1,p3)) 
		   + gammag + Kg )/CA 
	       )
	   /*
...
double Ioperator_gqb(const FourMomentum & p1, const FourMomentum & p2, 
  ...
  EvalLOfqq(f, -p3,p2,-p1,kq,kqb);

  iopgqb(Mij,p1,p2,p3,kq,kqb);

  /*
   *  Sum over the polarization and square it:
   */
  EvalSMEqq(S, -p3, p2, -p1,q1,q2,r1,r2);

  for (int tspin=0; tspin < 2; tspin++) {

    if (tspin == 1) 
      EvalSMEqq(S,-p3, p2, -p1,q1,q2,r2,r1);

    if (tspin == 2) 
      EvalSMEqq(S,-p3, p2, -p1,q2,q1,r1,r2);

    if (tspin == 3) 
      EvalSMEqq(S,-p3, p2, -p1,q2,q1,r2,r1);
...
double gqbttqb2(const FourMomentum & p1, const FourMomentum & p2, 
  ...
  /*
   * g(p1) qb(p2) --> t(kq) tb(kqb) + qb(p3) is obtained from
   * 
...
double gqbttqb2virt(const FourMomentum & p1,const FourMomentum & p2,
  ...
  if ( !initialized ) {
    ...
    cout << "# -------------------------------------------------------------"
	 << endl;
    cout << "# gqbttqb2virt: Initialize color matrix..."; 
    LOcolorqq(CLO);
    initialized = true;
    cout << " done." << endl;
    cout << "# -------------------------------------------------------------"
   ...
   */
  try {
    pentagon_qq(fivepoint,-p3,p2,-p1,kq,kqb, 0.0); 
  ...
  evaluatePVcoefficients(PVcoeffs,-p3,p2,-p1,kq,kqb,Bptr,Cptr,Dptr);

  MasslessFermionLoops_qq(f_nfl,-p3,p2,-p1,kq,kqb,Bptr,Cptr,Dptr,1.0 );   

  TopLoops_qq(f_top,-p3,p2,-p1,kq,kqb,Bptr,Cptr,Dptr, 1.0); 

  RemainingDiags_qq(rest,-p3,p2,-p1,kq,kqb,Bptr,Cptr,Dptr, 1.0);

  Ghosts_qq(ghosts,-p3,p2,-p1,kq,kqb,Bptr,Cptr,Dptr, 1.0);

  masscounterterm_qq(mct,-p3,p2,-p1,kq,kqb, 1.0);

  EvalLOfqq(flo, -p3, p2, -p1, kq, kqb);
  ...
  // 2 (spin) * 2 (2Re())
#ifdef TESTBADPOINTS
  static EPSHisto epsgqb("EPS_large_gqb.top");
  epsgqb.fill(log(fabs(0.01*amp2/amplo2)));
#endif
  if ( fabs(0.01*amp2/amplo2) > 100. ) {
    cout << "# Large correction drop event \n";
    cout << "# born = " << amplo2 << " nlo_gqb = " << amp2*0.01 << endl;
    amp2 = 0.0;
  }
  return(4.0*amp2);
}
