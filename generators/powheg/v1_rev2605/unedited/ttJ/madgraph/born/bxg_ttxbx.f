      SUBROUTINE Sbxg_ttxbx(P1,ANS,ANSJK,ANSMUNU)
C  
...
C  
C FOR PROCESS : b~ g -> t t~ b~  
C  
C Crossing   1 is b~ g -> t t~ b~  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C  
      ...
      common/to_amps_bxg_ttxbx/  amp2,       jamp2
...
C 
      ...
      DATA (bflav(i),i=1,NEXTERNAL_BORN) / -5, 0, 6, -6, -5 /
      ...
      common/to_POWHEG_bxg_ttxbx/amp2jk,cCF,cDENOM,amphel,helindex
C          
C BEGIN CODE
C ----------
      NTRY=NTRY+1
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                helindex=ihel
                 T=bxg_ttxbx(P ,NHEL(1,IHEL),JC(1))            
               DO JJ=1,nincoming
...
C 
      ...
      REAL*8 FUNCTION bxg_ttxbx(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C               T[ 1,4]T[ 3,5,2]                                        
      ...
      DATA (CF_CLBS(i,2  ,1,3),i=1  ,4  ) /  1, -8,  6, -3/             
C               T[ 3,5]T[ 1,4,2]                                        
      DATA Denom_CLBS(3  ,1,3)/         9/                                    
      DATA (CF_CLBS(i,3  ,1,3),i=1  ,4  ) / -3,  6, -8,  1/             
C               T[ 3,5,2]T[ 1,4]                                        
      DATA Denom_CLBS(4  ,1,3)/         9/                                    
      DATA (CF_CLBS(i,4  ,1,3),i=1  ,4  ) / -3, -3,  1,  1/             
C               T[ 3,5]T[ 1,4,2]                                        
C  
      DATA Denom_CLBS(1  ,1,4)/        18/                                    
      DATA (CF_CLBS(i,1  ,1,4),i=1  ,4  ) /-56,  7,-57,  6/             
C               T[ 1,4]T[ 3,5,2]                                        
      DATA Denom_CLBS(2  ,1,4)/        18/                                    
      DATA (CF_CLBS(i,2  ,1,4),i=1  ,4  ) /  7,  7,  6,  6/             
C               T[ 3,5]T[ 1,4,2]                                        
      DATA Denom_CLBS(3  ,1,4)/        18/                                    
      DATA (CF_CLBS(i,3  ,1,4),i=1  ,4  ) /-57,  6,-56,  7/             
C               T[ 3,5,2]T[ 1,4]                                        
...
C  
      ...
      DATA (CF_CLBS(i,1  ,1,5),i=1  ,4  ) /  8, -1,  7, -2/             
C               T[ 1,4]T[ 3,5,2]                                        
      DATA Denom_CLBS(2  ,1,5)/        18/                                    
      DATA (CF_CLBS(i,2  ,1,5),i=1  ,4  ) / -1,  8, -2,  7/             
C               T[ 3,5]T[ 1,4,2]                                        
      DATA Denom_CLBS(3  ,1,5)/        18/                                    
      DATA (CF_CLBS(i,3  ,1,5),i=1  ,4  ) /  7, -2, -1,-10/             
C               T[ 3,5,2]T[ 1,4]                                        
      DATA Denom_CLBS(4  ,1,5)/        18/                                    
      DATA (CF_CLBS(i,4  ,1,5),i=1  ,4  ) / -2,  7,-10, -1/             
C               T[ 3,5]T[ 1,4,2]                                        
C  
      DATA Denom_CLBS(1  ,2,3)/         2/                                    
      DATA (CF_CLBS(i,1  ,2,3),i=1  ,4  ) / -8,  1, -7,  2/             
C               T[ 1,4]T[ 3,5,2]                                        
      DATA Denom_CLBS(2  ,2,3)/         2/                                    
      DATA (CF_CLBS(i,2  ,2,3),i=1  ,4  ) /  1,  1,  0,  0/             
C               T[ 3,5]T[ 1,4,2]                                        
      DATA Denom_CLBS(3  ,2,3)/         2/                                    
      DATA (CF_CLBS(i,3  ,2,3),i=1  ,4  ) / -7,  0, -7,  0/             
C               T[ 3,5,2]T[ 1,4]                                        
      DATA Denom_CLBS(4  ,2,3)/         1/                                    
      DATA (CF_CLBS(i,4  ,2,3),i=1  ,4  ) /  1,  0,  0, -1/             
C               T[ 3,5]T[ 1,4,2]                                        
C  
...
C               T[ 1,4]T[ 3,5,2]                                        
      ...
      DATA (CF_CLBS(i,2  ,2,4),i=1  ,4  ) /  1, -8,  2, -7/             
C               T[ 3,5]T[ 1,4,2]                                        
      DATA Denom_CLBS(3  ,2,4)/         1/                                    
      DATA (CF_CLBS(i,3  ,2,4),i=1  ,4  ) /  0,  1, -1,  0/             
C               T[ 3,5,2]T[ 1,4]                                        
      DATA Denom_CLBS(4  ,2,4)/         2/                                    
      DATA (CF_CLBS(i,4  ,2,4),i=1  ,4  ) /  0, -7,  0, -7/             
C               T[ 3,5]T[ 1,4,2]                                        
C  
      DATA Denom_CLBS(1  ,2,5)/         2/                                    
      DATA (CF_CLBS(i,1  ,2,5),i=1  ,4  ) / -7,  0, -7,  0/             
C               T[ 1,4]T[ 3,5,2]                                        
      DATA Denom_CLBS(2  ,2,5)/         1/                                    
      DATA (CF_CLBS(i,2  ,2,5),i=1  ,4  ) /  0, -1,  1,  0/             
C               T[ 3,5]T[ 1,4,2]                                        
      DATA Denom_CLBS(3  ,2,5)/         2/                                    
      DATA (CF_CLBS(i,3  ,2,5),i=1  ,4  ) / -7,  2, -8,  1/             
C               T[ 3,5,2]T[ 1,4]                                        
...
C ----------
      ...
      DO I = 1, NCOLOR
          ...
          DO J = 1, NCOLOR
              ...
              cCF(J,I)=CF(J,I)
          ENDDO
          bxg_ttxbx =bxg_ttxbx+ZTEMP*DCONJG(JAMP(I))/DENOM(I)
C 
         do jjj=1,NEXTERNAL_BORN
