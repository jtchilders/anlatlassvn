...
c     Added this 'if' to be sure that no division by zero occurs
     ...
     #(r0(alr,jexp-1)-r0c(alr,jexp-1)),r0c(alr,jexp)/r0(alr,jexp),flag
      ...
      character * 200 stringa,stringb
      integer lstringa,lstringb
      if(ini) then
         do alr=1,flst_nalr
            equivto(alr)=-1
         enddo
...
c     check if res(j,alr) is finite and different form zero 
                  if ((.not.pwhg_isfinite(res(j,alr))).or.(res(j
     $                 ,alr).eq.0d0)) then
                     write(*,*) "error in sigreal_rad: real must"
...
c     < 0 for unequal:
            ...
            if(verbose) then
               call newunit(iun)
               open(unit=iun,file='IndependentRealRadList',status
     $              ='unknown')
               do alr=1,flst_nalr
                  call from_number_to_madgraph (nlegreal,flst_alr(1,alr)
     $                 ,flst_emitter(alr),stringa)
                  do lstringa=len(stringa),0,-1
                     if(stringa(lstringa:lstringa).ne.' ') goto 13
                  enddo
 13               continue
                  if (equivto(alr).eq.-1) then 
                     call from_number_to_madgraph (nlegborn,flst_born(1
     $                    ,flst_alr2born(alr)),-1,stringb)
                     do lstringb=len(stringb),0,-1
                        if(stringb(lstringb:lstringb).ne.' ') goto 14
                     enddo
 14                  continue
                     write(iun,'(a)') stringa(1:lstringa)/
     $                    /"  uborn --> "//stringb(1:lstringb)
                  endif
               enddo
               close(iun)
            endif
...
c     If the emitter is in the final state, and if the emitted and
c     emitter are both gluons, supply a factor 2 * E_em^par_2gsupp
c     /(E_em^par_2gsupp + E_rad^par_2gsupp) 
               if(em.gt.2) then
...
c     generate "nmomset" random real-phase space configurations
     ...
     1                 flst_alr(1,alr),preal(0,1,j),res(j,alr))
c     check if res(j,alr) is finite and different form zero 
                  if ((.not.pwhg_isfinite(res(j,alr))).or.(res(j ,alr)
     $                 .eq.0d0)) then
                     write(*,*) "error in sigreal_btl0: real must"
                     write(*,*) " be finite during smartsig evaluation"
                     stop
                  endif
               enddo
...
c     < 0 for unequal:
            ...
            if(verbose) then
               call newunit(iun)
               open(unit=iun,file='IndependentRealList',status
     $              ='unknown')
               do alr=1,flst_nalr
                  call from_number_to_madgraph (nlegreal,flst_alr(1,alr)
     $                 ,flst_emitter(alr),stringa)
 ...
 11               continue
     ...
     $                    ,flst_alr2born(alr)),-1,stringb)
                     do lstringb=len(stringb),0,-1
                        if(stringb(lstringb:lstringb).ne.' ') goto 12
                     enddo
 12                  continue
                     write(iun,'(a)') stringa(1:lstringa)/
     $                    /"  uborn --> "//stringb(1:lstringb)
...
c include pdf's
      ...
      subroutine fillmomenta(nlegs,nmomset,masses,p)
      implicit none
      include 'pwhg_math.h'
      integer nlegs,nmomset
      real * 8 p(0:3,nlegs,nmomset),masses(nlegs)
      integer mu,j,k
      real * 8 pl,en,ran,phi(nlegs),theta(nlegs),pmod
      real * 8 random
      external random
      do j=1,nmomset
         ran=0.25d0+random()*0.5d0
         do k=3,nlegs-1
C     First generate directions in order to avoid collinear singularities       
            phi(k)= 2*pi/(nlegs-2) * ( ran + k )
            theta(k)= pi/(nlegs-2) * ( ran + k )
C     Avoid soft singularities taking all momenta between 50 and 100 GeV
            pmod=50d0*(1+random())
            p(1,k,j)=pmod*sin(theta(k))*cos(phi(k)) 
            p(2,k,j)=pmod*sin(theta(k))*sin(phi(k))
            p(3,k,j)=pmod*cos(theta(k))
         enddo
         p(3,nlegs,j)=50d0*(1+random())
         p(1,nlegs,j)=0
         p(2,nlegs,j)=0
         do k=3,nlegs-1
            p(1,nlegs,j)=p(1,nlegs,j)-p(1,k,j)
            p(2,nlegs,j)=p(2,nlegs,j)-p(2,k,j)
         enddo
         pl=0
         en=0
         do k=3,nlegs
c We should have the masses as a function of flavour id, somewhere!
            p(0,k,j)=sqrt(masses(k)**2+
     #         p(1,k,j)**2+p(2,k,j)**2+p(3,k,j)**2)
            pl=pl+p(3,k,j)
            en=en+p(0,k,j)
         enddo
         p(0,1,j)=(en+pl)/2
         ...
         p(2,1,j)=0
         p(2,2,j)=0
c         print *,((p(mu,k,j),mu=0,3),"\n",k=1,nlegs),"\n"
c         call checkmomzeronew(nlegs,p(0,1,j))
c         call checkmassnew(nlegs,p(0,1,j))
      enddo
      end
