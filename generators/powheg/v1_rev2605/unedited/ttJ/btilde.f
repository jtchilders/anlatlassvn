...
c     for x_1 and x_2, and take away an overall azimuth
     ...
     #     resreal(maxprocborn),rescoll(maxprocborn)
      ...
      enddo
      if(ifirst.eq.0) then
         call increasecnt("btilde calls ifirst=0")
...
c     sets xscaled, y, phi in kinematics common block
         ...
         if (.not.flg_bornonly) then
            ...
            call btildecoll(xrad(1),rescoll,www)
...
c     to the NLO analysis routine.
         call increasecnt("btilde calls ifirst=1")
         wwwtot=wwwtot+www
         if (.not.flg_bornonly) then
...
c     accumulate values in results
      ...
      elseif(ifirst.eq.2) then
         call increasecnt("btilde calls ifirst=2")
         if(flg_nlotest) then
c output Born
            totborn=0d0
            do j=1,flst_nborn
...
c if an error occurred during the previous steps, empty the histograms
            if(error_occurred) then 
               call pwhgdiscardup
c else perform the closing call to end a sequence of correlated
c events in the analysis routines.
            else 
               call pwhgaccumup
            endif
         endif
