      ...
      real * 8 c(-nparton:nparton),gamma(-nparton:nparton), 
     +         gammap(-nparton:nparton)
     ...
     #     loglog
      ...
      real * 8 tmp,tmpew,Qem,Iem,chi,chj
...
c from 2.100 of FNO2007
      if(ini) then
         ...
         enddo
CCCC    !!!!!!!! WARNING  !!!!!!!!!!     CCCC
C     virtual corrections for photon        C
C     aren't taken into account             C
         do j=7,nparton
            c(j)=0d0
            gamma(j)=0d0
            gammap(j)=0d0
...
c WEW photonic contribution only if incoming particle are not gluon
      Qem = 0d0
      ...
      llew = log(par_csicut**2*s/mudim2)
      do jb=1,flst_nborn
         fl1=flst_born(1,jb)
         fl2=flst_born(2,jb)
c     initial-state parton contribution
         if (is_coloured(fl1).and.is_coloured(fl2)) then
             Q=-log(st_mufact2/st_muren2)*(gamma(fl1)+2*c(fl1)*
     1       log(par_csicut) +  gamma(fl2)+2*c(fl2)*log(par_csicut))
         endif
c WEW photonic contribution
         chi=chargeofparticle(fl1)
         chj=chargeofparticle(fl2)
         if (is_charged(fl1).and.is_charged(fl2)) then
            Qem=-log(st_mufact2/mudim2)*( 
     1               (gamma(fl1)+2*c(fl1)*
     2                  log(par_csicut))*chi**2/cf
     3             + (gamma(fl2)+2*c(fl2)*
     4                  log(par_csicut))*chj**2/cf  )
         endif
c     loop on final-state massless partons
     ...
     1                 (ll-0.5*Intm_ep(kn_cmpborn(0,leg)))               
               endif
            endif
c WEW
            if (chargeofparticle(fl).ne.0) then
               if(kn_masses(leg).eq.0) then
...
c     both particles are colored
     ...
     #       is_coloured(flst_born(legj,jb))     ) then
...
c     massive-massive case
     ...
     #            *br_bornjk(legi,legj,jb)
         ...
         chi=chargeofparticle(flst_born(legi,jb))
         chj=chargeofparticle(flst_born(legj,jb))

c     both particles are charged
         if (chi.ne.0.and.chj.ne.0) then
c     massless-massless case
            if (kn_masses(legi).eq.0.and.kn_masses(legj).eq.0) then
               kij=dotp(kn_cmpborn(0,legi),kn_cmpborn(0,legj))
               eij=kn_cmpborn(0,legi)*kn_cmpborn(0,legj)
     ...
     #              -ddilog(kij/(2*eij))+1d0/2*log(kij/(2*eij))**2
     #              -loglog)*
     #              br_born(jb)*chi*chj*(-1)*
     #              sigma(flst_born(legi,jb),legi)*
     #              sigma(flst_born(legj,jb),legj)
            endif
c     massless-massive case
            if (kn_masses(legi).eq.0.and.kn_masses(legj).gt.0) then
               Iem = Iem + (0.5*(0.5*llew**2 - pi**2/6)
     #          +0.5*Int0m_0(kn_cmpborn(0,legi),kn_cmpborn(0,legj))*llew
     #          -0.5*Int0m_ep(kn_cmpborn(0,legi),kn_cmpborn(0,legj)))*
     #              br_born(jb)*chi*chj*(-1)*
     #              sigma(flst_born(legi,jb),legi)*
     #              sigma(flst_born(legj,jb),legj)
...
c     massive-massless case
            if (kn_masses(legi).gt.0.and.kn_masses(legj).eq.0) then
               Iem = Iem + (0.5*(0.5*llew**2 - pi**2/6)
     #          +0.5*Int0m_0(kn_cmpborn(0,legj),kn_cmpborn(0,legi))*llew
     #          -0.5*Int0m_ep(kn_cmpborn(0,legj),kn_cmpborn(0,legi)))*
     #              br_born(jb)*chi*chj*(-1)*
     #              sigma(flst_born(legi,jb),legi)*
     #              sigma(flst_born(legj,jb),legj)
...
c WEW
     ...
     1         + (Qem+Iem)*physpar_aem )/(2*pi)
     2       *pdfb1(fl1)*pdfb2(fl2)*kn_jacborn
         tot=tot+resvirt(jb)
      enddo
...
c the range in phi is 0<phi<pi.
      ...
      if (abs(beta-1d0).gt.1d-8) then
          Intmm_0=log((1+beta)/(1-beta))/beta
      else
          Intmm_0=Log(4d0)-Log(dcmplx(g2-1d0))-Log(dcmplx(k2-1d0))
     +           +2d0*Log(dcmplx(kg-1d0))
      endif
      end
