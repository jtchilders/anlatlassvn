...
c            kinematic regions).
      ...
      include 'pwhg_kn.h'
      real * 8 rc(maxalr)
      real * 8 pdf1(-nparton:nparton),pdf2(-nparton:nparton)
      integer alr
...
c     Then rotate it counterclockwise around the em direction
     ...
     #/kn_cmpborn(3,em)
      ...
      include 'pwhg_kn.h'
      real * 8 rc(maxalr)
      real * 8 pdf1(-nparton:nparton),pdf2(-nparton:nparton),x1,x2
      integer alr
...
c provide multiplicity of emitter in underlyng Born
      ...
      integer iub,emflav,raflav,mi,nu
     ...
     #           0d0, 0d0, 0d0,-1d0/
      ...
      if(emflav.eq.0.and.raflav.eq.0) then
         ...
         do mi=0,3
            do nu=0,3
               ap=ap+(-gtens(mi,nu)*(x+x*(1-x)**2)
     #+2*(1-x)**2/x*
     #(kperp(mi)*kperp(nu)))*br_bmunu(mi,nu,i,iub)
            enddo
         enddo
         ap=ap*2*ca
CCCCC     !!!!!!!!!!! WARNING !!!!!!!!!!!     CCCCCCCCC
C    for NC DY the quark-quark splitting is always    C
C    due to the photon. uncomment these  lines below  C
C    for any other process                            C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
c      elseif(raflav-emflav.eq.0) then
c         ap=0d0
c         do mi=0,3
c            do nu=0,3
c               ap=ap+(-gtens(mi,nu)*x
c     #+4*(1-x)/x*(kperp(mi)*kperp(nu)))*br_bmunu(mi,nu,i,iub)
c            enddo
c         enddo
c         ap=ap*cf*(1-x)
      elseif(raflav-emflav.eq.0) then
         ...
         do mi=0,3
            do nu=0,3
               ap=ap+(-gtens(mi,nu)*x
     #+4*(1-x)/x*(kperp(mi)*kperp(nu)))*br_bmunu(mi,nu,i,iub)
            enddo
         enddo
...
c     using csi=1-x we get
      if( raflav.eq.22.or.emflav.eq.22.or.
     $      emflav-raflav.eq.0 ) then
...
c - 1 for incoming antifermion or outgoing fermion
      ...
      integer sigma
      external dotp,chargeofparticle,sigma
