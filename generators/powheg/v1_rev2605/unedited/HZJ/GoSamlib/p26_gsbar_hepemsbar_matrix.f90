module p26_gsbar_hepemsbar_matrix
...
contains
   ...
   pure function prefactor()
      ...
      use p12_sbars_hepemg_model, only: NC
      use p12_sbars_hepemg_kinematics, only: &
      & in_helicities, symmetry_factor
      use p12_sbars_hepemg_color, only: incolors
      implicit none
   ...
   subroutine     initgolem(is_first)
      use p12_sbars_hepemg_matrix, only: orig_initgolem => initgolem
      implicit none
   ...
   subroutine     exitgolem(is_last)
      use p12_sbars_hepemg_matrix, only: orig_exitgolem => exitgolem
      implicit none
   ...
   subroutine     samplitude(vecs, scale2, amp, ok, h)
      use p12_sbars_hepemg_matrix, only: orig_sub => samplitude
      implicit none
   ...
   function     samplitudel0(vecs, h) result(amp)
      use p12_sbars_hepemg_matrix, only: orig_func => samplitudel0
      implicit none
      real(ki), dimension(6, 4), intent(in) :: vecs
   ...
   function     samplitudel1(vecs,scale2,ok,rat2,h) result(amp)
      use p12_sbars_hepemg_matrix, only: orig_func => samplitudel1
      implicit none
      real(ki), dimension(6, 4), intent(in) :: vecs
...
end module p26_gsbar_hepemsbar_matrix
