module     p16_bbarb_hepemg_matrix
   ...
   use p16_bbarb_hepemg_model, only: Nf, NC, sqrt2, init_functions
   use p16_bbarb_hepemg_color, only: TR, CA, CF, numcs, &
     & incolors, init_color
   use p16_bbarb_hepemg_diagramsh0l0, only: amplitude0l0 => amplitude
   use p16_bbarb_hepemg_amplitudeh0, only: samplitudeh0l1 => samplitude, &
        &   finite_renormalisation0 => finite_renormalisation
   use p16_bbarb_hepemg_diagramsh1l0, only: amplitude1l0 => amplitude
   use p16_bbarb_hepemg_amplitudeh1, only: samplitudeh1l1 => samplitude, &
        &   finite_renormalisation1 => finite_renormalisation
   use p16_bbarb_hepemg_diagramsh2l0, only: amplitude2l0 => amplitude
   use p16_bbarb_hepemg_amplitudeh2, only: samplitudeh2l1 => samplitude, &
        &   finite_renormalisation2 => finite_renormalisation
   use p16_bbarb_hepemg_diagramsh3l0, only: amplitude3l0 => amplitude
   use p16_bbarb_hepemg_amplitudeh3, only: samplitudeh3l1 => samplitude, &
        &   finite_renormalisation3 => finite_renormalisation
   use p16_bbarb_hepemg_dipoles, only: insertion_operator
...
contains
               ...
               if(PSP_verbosity .ge. 1) then
                  ...
                  if(spprec2 .le. PSP_chk_threshold2 .and. spprec2 .gt. -10000) then
                     write(42,'(4x,A15,A16,A18,A3)') "<process name='", &
                          &   "p16_bbarb_hepemg", "' problem='sinpole","'/>"
                  else if(abs(kfac) > PSP_chk_kfactor) then
                     write(42,'(4x,A15,A16,A18,A3)') "<process name='", &
                          &   "p16_bbarb_hepemg", "' problem='kfactor","'/>"
                  end if
   ...
   subroutine     samplitudel01(vecs, scale2, amp, rat2, ok, h)
      ...
      use p16_bbarb_hepemg_dipoles, only: pi
      implicit none
      real(ki), dimension(6, 4), intent(in) :: vecs
      real(ki), intent(in) :: scale2
      ...
      case default
         ! not implemented
         print*, "In p16_bbarb_hepemg_matrix:"
   ...
   function     samplitudel0(vecs, h) result(amp)
      use p16_bbarb_hepemg_config, only: logfile
      use p16_bbarb_hepemg_kinematics, only: init_event
      implicit none
      real(ki), dimension(6, 4), intent(in) :: vecs
   ...
   pure subroutine color_correlated_lo(color_vector,res)
      use p16_bbarb_hepemg_color, only: T1T1, &
      & T1T2, &
      & T1T6, &
      & T2T2, &
      & T2T6, &
   ...
   pure function square_0l_0l_sc(color_vector1, color_vector2) result(amp)
      use p16_bbarb_hepemg_color, only: cmat => CC
      implicit none
...
end module p16_bbarb_hepemg_matrix
