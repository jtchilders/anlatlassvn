      SUBROUTINE Sdxd_epemuuxg(P1,ANS)
C  
...
C  
C FOR PROCESS : d~ d -> e+ e- u u~ g  
C  
C Crossing   1 is d~ d -> e+ e- u u~ g  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C  
      ...
      common/to_amps_dxd_epemuuxg/  amp2,       jamp2
...
C ----------
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=dxd_epemuuxg(P ,NHEL(1,IHEL),JC(1))            
               DO JJ=1,nincoming
     ...
     &             NHEL(JJ,IHEL).EQ.INT(SIGN(1d0,POL(JJ)))) THEN
      ...
      double precision FUNCTION dxd_epemuuxg(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
     ...
     &             +AMP(  32)+AMP(  43)+AMP(  44)+AMP(  47)+AMP(  48)
      ...
      DO I = 1, NCOLOR
          ...
          ENDDO
          dxd_epemuuxg =dxd_epemuuxg+ZTEMP*conjg(JAMP(I))/DENOM(I)   
      ENDDO
      Do I = 1, NGRAPHS
