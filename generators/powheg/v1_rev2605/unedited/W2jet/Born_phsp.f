...
c     set initial- and final-state masses for Born and real
         ...
         if(mllmin.lt.0) then
            write(*,*) ' Warning: mllmin not given, set to 0 GeV'
            mllmin=0
         endif
         if(bwcutoff.gt.0) then
            ph_Wmass2low=max(wmass-bwcutoff*wwidth,mllmin)**2
            ph_Wmass2high=min(wmass+bwcutoff*wwidth,sqrt(kn_sbeams))**2
         else
            ph_Wmass2low=mllmin**2
            ph_Wmass2high=kn_sbeams
         endif
         ptborncut=powheginput("#ptborncut")
         if(ptborncut.lt.0) then 
            write(*,*) ' ptborncut not given, set to 0.01 GeV'
            ptborncut=1d-2
         endif
...
c     minimal final state mass 
      kn_minmass=brkn_ktmin + sqrt(brkn_ktmin**2 + ph_Wmass2low)
...
c csimax depends upon y in this case.
     ...
     1     -brkn_cmpborn(3,j)**2
      ...
      include 'pwhg_flg.h'
      real * 8 fact,ptmin
      real * 8 pt5,pt6,pt56,dotp,ptmin2,s56
      real * 8 powheginput
      external powheginput
      logical ini
      data ini/.true./
     ...
     1        *  kn_cmpborn(0,5)*kn_cmpborn(0,6)
     2        /(kn_cmpborn(0,5)**2+kn_cmpborn(0,6)**2)
C         fact=((1/ptmin**2)/(1/pt5+1/pt6+1/pt56+1/ptmin**2))**2
         fact=((1/ptmin**2)/(1/pt5+1/pt6+1/pt56+1/ptmin**2))

CC     alternative suppression factor as in Re 1204.5433 
C         s56 = 2*dotp(kn_cmpborn(0,5),kn_cmpborn(0,6))
C         ptmin2=ptmin**2
C         fact = (pt5/(pt5+ptmin2))**2*(pt6/(pt6+ptmin2))**2
C     $        *(s56/(s56+ptmin2))**2
      ...
      if (ini) then
         ...
         if (st_renfact .gt. 0d0) 
     .        write(*,*) 'Renormalization scale rescaled by', st_renfact
         if (st_facfact .gt. 0d0) 
     .        write(*,*) 'Factorization scale rescaled by  ', st_facfact
         ini=.false.
      endif
      ...
      if (runningscales.eq.1) then
         if(flg_btildepart.eq.'b'.or.flg_btildepart.eq.'c') then
            ...
            ptwsq=(kn_pborn(1,3)+kn_pborn(1,4))**2
     $           +(kn_pborn(2,3)+kn_pborn(2,4))**2
            Ht=sqrt(zmass**2+ptwsq)+pt1+pt2
         elseif(flg_btildepart.eq.'r') then
            ...
            ptwsq=(kn_preal(1,3)+kn_preal(1,4))**2
     $           +(kn_preal(2,3)+kn_preal(2,4))**2
            Ht=sqrt(zmass**2+ptwsq)+pt1+pt2+pt3
         endif
         mur=Ht/2d0
         muf=mur
      else
