...
c     possible in the subroutine put_on_mass_shell
c     (see the !:!:! marks).
cccccccccccccccccccccccccccccccccccccc
...
c-------------------------------
c     tdecay_offshell knows if the current event is a born or a real by means of nup
...
c     Everything already performed by tdecay_offshell
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine tdecay_offshell(klab_dec)
...
c     gd -> tuxd
     ...
     #(three_ch(idup_loc(4)).eq.-2)) then
               call Sgb_tuxd_T(klab_undec,amp2mad)
c     gdx -> tuxdx
            elseif((three_ch(idup_loc(2)).eq.1).and.
     #(three_ch(idup_loc(4)).eq.-2)) then
               call Sgdx_tuxbx_T(klab_undec,amp2mad)
c     gu -> tddx
            elseif((three_ch(idup_loc(2)).eq.2).and.
     #(three_ch(idup_loc(4)).eq.-1)) then
               call Sgu_tdbx_T(klab_undec,amp2mad)
            else
...
c     dg -> tuxd
            if((three_ch(idup_loc(1)).eq.-1).and.
     #(three_ch(idup_loc(4)).eq.-2)) then
               call Sbg_tuxd_T(klab_undec,amp2mad)
c     dxg -> tuxdx
            elseif((three_ch(idup_loc(1)).eq.1).and.
     #(three_ch(idup_loc(4)).eq.-2)) then
               call Sdxg_tuxbx_T(klab_undec,amp2mad)
c     ug -> tddx
            elseif((three_ch(idup_loc(1)).eq.2).and.
     #(three_ch(idup_loc(4)).eq.-1)) then
               call Sug_tdbx_T(klab_undec,amp2mad)
            else
...
c     ddx
            if ((three_ch(idup_loc(1)).eq.-1).and.
     #(three_ch(idup_loc(2)).eq.1)) then
               call Sbdx_tuxg(klab_undec,amp2mad)
c     dxd
            elseif ((three_ch(idup_loc(1)).eq.1).and.
     #(three_ch(idup_loc(2)).eq.-1)) then
               call Sdxb_tuxg(klab_undec,amp2mad)
c     ud
            elseif ((three_ch(idup_loc(1)).eq.2).and.
     #(three_ch(idup_loc(2)).eq.-1)) then
               call Sub_tdg(klab_undec,amp2mad)
c     du
            elseif ((three_ch(idup_loc(1)).eq.-1).and.
     #(three_ch(idup_loc(2)).eq.2)) then
               call Sbu_tdg(klab_undec,amp2mad)
            else
...
c     boost 3rd and 'non-FKS-parton' momentum in their CM frame
c     Denote this system as 3rec
     ...
     $(kcm_undec(0,3)+kcm_undec(0,4))  !:!
      enddo
      do ileg=3,4 !:!
         call boost(beta_cm_to_3reccm,kcm_undec(0,ileg),
...
c     extreme (large) values of m67 seem to cause problems to herwig; avoid them!
...
c     boost the new 3rd and recth momenta back in the CM frame,
...
c     3rec system (i.e. the 3 momentum of the radiated parton)
     ...
     $        /E3rec_cm  !:!
      enddo
      do ileg=3,4 !:!
         call boost(beta_3reccm_to_cm,k3reccm_dec(0,ileg),
     $        kcm_dec(0,ileg))
      enddo
...
c     ddx
         if ((three_ch(idup_loc(1)).eq.-1).and.
     #(three_ch(idup_loc(2)).eq.1)) then
            call sbdx_epvebux(kbcm_mad,amp2mad)
c     dxd
         elseif ((three_ch(idup_loc(1)).eq.1).and.
     #(three_ch(idup_loc(2)).eq.-1)) then
            call sdxb_epvebux(kbcm_mad,amp2mad)
c     ud
         elseif ((three_ch(idup_loc(1)).eq.2).and.
     #(three_ch(idup_loc(2)).eq.-1)) then
            call sub_epvebd(kbcm_mad,amp2mad)
c     du
         elseif ((three_ch(idup_loc(1)).eq.-1).and.
     #(three_ch(idup_loc(2)).eq.2)) then
            call sbu_epvebd(kbcm_mad,amp2mad)
         else
...
c     gd -> tuxd
            if((three_ch(idup_loc(2)).eq.-1).and.
     #(three_ch(idup_loc(4)).eq.-2)) then
               call Sgb_epvebuxd_T(krcm_mad,amp2mad)
c     gdx -> tuxdx
            elseif((three_ch(idup_loc(2)).eq.1).and.
     #(three_ch(idup_loc(4)).eq.-2)) then
               call Sgdx_epvebuxbx_T(krcm_mad,amp2mad)
c     gu -> tddx
            elseif((three_ch(idup_loc(2)).eq.2).and.
     #(three_ch(idup_loc(4)).eq.-1)) then
               call Sgu_epvebdbx_T(krcm_mad,amp2mad)
            else
...
c     dg -> tuxd
            if((three_ch(idup_loc(1)).eq.-1).and.
     #(three_ch(idup_loc(4)).eq.-2)) then
               call Sbg_epvebuxd_T(krcm_mad,amp2mad)
c     dxg -> tuxdx
            elseif((three_ch(idup_loc(1)).eq.1).and.
     #(three_ch(idup_loc(4)).eq.-2)) then
               call Sdxg_epvebuxbx_T(krcm_mad,amp2mad)
c     ug -> tddx
            elseif((three_ch(idup_loc(1)).eq.2).and.
     #(three_ch(idup_loc(4)).eq.-1)) then
               call Sug_epvebdbx_T(krcm_mad,amp2mad)
            else
...
c     ddx
            if ((three_ch(idup_loc(1)).eq.-1).and.
     #(three_ch(idup_loc(2)).eq.1)) then
               call Sbdx_epvebuxg(krcm_mad,amp2mad)
c     dxd
            elseif ((three_ch(idup_loc(1)).eq.1).and.
     #(three_ch(idup_loc(2)).eq.-1)) then
               call Sdxb_epvebuxg(krcm_mad,amp2mad)
c     ud
            elseif ((three_ch(idup_loc(1)).eq.2).and.
     #(three_ch(idup_loc(2)).eq.-1)) then
               call Sub_epvebdg(krcm_mad,amp2mad)
c     du
            elseif ((three_ch(idup_loc(1)).eq.-1).and.
     #(three_ch(idup_loc(2)).eq.2)) then
               call Sbu_epvebdg(krcm_mad,amp2mad)
            else
...
c     event left massless
      if(MC_mass.eq.0) then
         ...
         enddo
         if(tdecayflag) then
            call pickwdecay(iw1,mdecw1,iw2,mdecw2,dummy)
c     e
            decids(1)=iw1
c     ve
            decids(2)=iw2
...
!      if(nup.eq.4) then
     ...
     #(totpy.gt.1d-5).or.(totpz.gt.1d-5)) then
            ...
            print*, 's partonic',shat,m34**2,2d0*dotp(xklab_os(0,1),
     $       xklab_os(0,2)),175**2+2d0*dotp(xklab_os(0,3),xklab_os(0,4))
            do ileg=1,2
