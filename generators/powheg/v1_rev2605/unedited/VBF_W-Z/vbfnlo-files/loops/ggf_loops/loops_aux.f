...
c     changed 27.07.2012 for POWHEG implementation by
c     F. Schissler
c
c  determine the Passarino-Veltman tensor decomposition for the 
...
c       decomposition (used in SUBROUTINE DDtens51m in ggf_amp_aux.F) 
     ...
     &       +p1sq*p2p3**2+p2sq*p1p3**2+p3sq*p1p2**2
      ...
      elseif ((abs(deti).lt.1d-30).and.(kn_jacborn.eq.0d0.or.flg_weightedev)) then
          return ! just return, since PS-point is omitted anyway
      endif
      deti = 1/deti
c
...
c  
      ...
      parameter (pi2o6=pi**2/6d0,pi2o2=pi**2/2d0,tiny=1d-5)
...
c                  endif
      ...
      elseif (izero.eq.2) then
         ...
         C0t = -res/qsq(1)
      elseif ((izero.eq.3).and.((kn_jacborn.eq.0d0).or.(flg_weightedev))) then
         C0t=0d0
      else
