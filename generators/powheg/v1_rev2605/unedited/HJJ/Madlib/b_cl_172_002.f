...
C  
C FOR PROCESS : g g -> h u u~ g  
C  
C BORN AMPLITUDE IS g g -> h u u~  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C          write (*,*) "momenta not the same in Born"
     ...
     &       nhel(2,ihel).eq.-1) THEN
                 T=B_CL_172_002(P1,NHEL(1,IHEL),IHEL,JC(1),T1)            
     ...
     &                 .NOT. GOODHEL(IHEL,IPROC)) THEN
      ...
      COMPLEX*16 FUNCTION B_CL_172_002(P,NHEL,HELL,IC,BORNTILDE)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
     ...
     &             -AMP(   8)
      DO I = 1, NCOLOR
          ...
          if (ihel.eq.-1) then
             B_CL_172_002=B_CL_172_002 +RTEMP
          elseif (ihel.eq.1) then
             B_CL_172_002=B_CL_172_002 +(0d0,1d0)*RTEMP
          endif
      ENDDO
      Do I = 1, NGRAPHS
