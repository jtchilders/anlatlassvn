...
c local variables, external functions etc.
     ...
     1           Cij134(2,4,8), Cij234(2,4,8)
      ...
      double precision dotp, psumsq
      external dotp, psumsq
...
c and the effective q1 and q2 for individual subsets of diagrams
      ...
      t(5) = t34
      t(6) = u34
      q1sq(1) = dotp(q1(0,1),q1(0,1))
      q2sq(1) = dotp(q2(0,1),q2(0,1))
      q1sq(2) = q2sq(1)
      q2sq(2) = q1sq(1)
      q1sq(5) = dotp(q1(0,5),q1(0,5))
      q2sq(5) = q2sq(1)
      q1sq(6) = q2sq(5)
      q2sq(6) = q1sq(5)
      do id = 3,4
      ...
      if (ldebug) then
         ...
         do id = 1,8
            print*,id,q1sq(id)/dotp(q1(0,id),q1(0,id)),
     1                q2sq(id)/dotp(q2(0,id),q2(0,id))
         enddo
      endif
...
c
      ...
      Cij(1,4) = 0
c
      C24 = -0.5d0*mt**2*C0 + 0.25d0*(1+b23-f1*C11-f2*C12)
         ...
         if (flambda.gt.0d0 .and. .not.ltaylor) then
            ...
            do i=1,3
               ...
               if (x.gt.1d0) then
                  ...
                  z = rz
                  logz = log(rz)
                  res = res + 2*Li2(z)+0.5d0*logz**2-pi2o3 
               ...
               elseif (x.gt.0d0) then
                  ...
                  z = rz
                  logz = log(-rz)
                  res = res + 2*Li2(z)+0.5d0*logz**2+pi2o6
...
c  
      complex*16 ipi, ieps, z1, z2, zlog, li2, D0t4
...
c
      ...
      D0t = (0.5d0*zlog**2 + 2*(li2(z1)+li2(z2)) - pi2o6)/(sl*tl)
c
      if (ldebug) then
...
c
      ...
      if (abs(dot0p).lt.0.01*abs(p1(0)*p2(0))) then  !section for small p1.p2
         print*,' old dotp = ',dot0p,' ratio = ',dot0p/p1(0)/p2(0)
         fac = p2(0)/p1(0)
         pt(1)=p2(1)-fac*p1(1)
         pt(2)=p2(2)-fac*p1(2)
         pt(3)=p2(3)-fac*p1(3)
         dot0p = (pt(1)**2+pt(2)**2+pt(3)**2)/(2*fac)
         print*,' new dotp = ',dot0p
      endif
      end
c
c----- dotp(p1,p2) = p1.p2 ----------------------------
c
c      function dotp(p1,p2)
c      implicit none
c      double precision dotp,p1(0:3),p2(0:3)
c      dotp = p1(0)*p2(0) - p1(1)*p2(1) - p1(2)*p2(2) - p1(3)*p2(3)
c      end
c
c----- psumsq(p1,p2) = (p1+p2).(p1+p2) -----------------
c
      function psumsq(p1,p2)
      implicit none
      double precision psumsq,p1(0:3),p2(0:3)
      psumsq = (p1(0)+p2(0))**2 - (p1(1)+p2(1))**2 -
     1         (p1(2)+p2(2))**2 - (p1(3)+p2(3))**2
      end

      real*8 function srt(x2)
      real*8 x2
      if (x2.gt.0d0) then
         srt = sqrt(x2)
