...
c Rotate k_n+1 around vec of an amount psi
      call mrotate(vec,sqrt(abs(1d0-cpsi**2)),cpsi,xk(1,n+1))
c Rotate k_j around vec of an amount psi1 in opposite direction
      call mrotate(vec,-sqrt(abs(1d0-cpsi1**2)),cpsi1,xk(1,j))
...
c (i.e. not through gen_real_phsp_isr_rad0)
      ...
      csi=kn_csi
      cth=y
      sth=sqrt(abs(1d0-cth**2))
      cph=cos(kn_azi)
      sph=sin(kn_azi)
      x1=xb1/sqrt(abs(1d0-csi))*
     &   sqrt(abs((2d0-csi*(1d0-y))/(2d0-csi*(1d0+y))))
      x2=xb2/sqrt(abs(1d0-csi))*
     &   sqrt(abs((2d0-csi*(1d0+y))/(2d0-csi*(1d0-y))))
      kn_x1=x1
      kn_x2=x2
      do mu=0,3
...
c construct transverse boost velocity
      ...
      betat=1/sqrt(abs(1d0+(kn_sreal*(1d0-csi))/pt2))
...
c Set up a unit vector orthogonal to p_em and to the z axis
      ...
      call mrotate(dir,sqrt(abs(1d0-y**2)),y,kn_softvec(1))
      do j=1,3
...
c Rotate kn_softvec around dir of an amount azi
      ...
      y=kn_y
      kn_softvec(0)=1
      kn_softvec(1)=sqrt(abs(1d0-y**2))*sin(kn_azi)
      kn_softvec(2)=sqrt(abs(1d0-y**2))*cos(kn_azi)
      kn_softvec(3)=y
      end
