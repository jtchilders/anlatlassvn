...
c to find the upper bounding envelope
            if(flg_storemintupb) then
               if(flg_fastbtlbound.and.iiret.eq.0) then
               ...
               prod=1
               if(flg_fastbtlbound.and.iiret.eq.0) prodrat=1
               do kdim=1,ndim
     ...
     1                 prodrat=prodrat*ymaxrat(ncell(kdim),kdim)
               enddo
               prod=(f/prod)
               if(flg_fastbtlbound.and.iret.eq.0)
...
c     the returned coordinate vector of the generated point
     ...
     1         ymax(nintervals,ndim),ymaxrat(nintervals,ndim),x(ndim)
      ...
      integer icalls,mcalls,kdim,kint,nintcurr,iret,ifirst,istep,iiret
...
c save random status for each iteration
      ...
      call getcurrentrandom(gen_seed,gen_n1,gen_n2)
      do kdim=1,ndim
         nintcurr=nintervals/ifold(kdim)
         r=random()
         do kint=1,nintcurr
...
c If imode=2, only the full contribution is needed
 ...
 5    continue
      ...
      do kdim=1,ndim
         ...
         x(kdim)=xgrid(icell(kdim)-1,kdim)+rand(kdim)*dx(kdim)
      enddo
      iiret = fun(x,vol,ifirst,istep,vfun,vfun0)
c if iiret is nonzero the function does not support the avatar function;
c do only one iteration
      if(iiret.ne.0) istep = 1
...
c get final value (x and vol not used in this call)
      iiret = fun(x,vol,2,istep,vfun,vfun0)
      f = vfun
      if(imode.eq.2) then
         return
      endif
