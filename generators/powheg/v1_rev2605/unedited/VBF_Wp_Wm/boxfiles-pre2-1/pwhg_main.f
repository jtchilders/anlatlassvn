...
c         open(unit=iun,file=pwgprefix(1:lprefix)//'counters.dat'
c     1     ,status='unknown')
c      else
c         open(unit=iun,file=pwgprefix(1:lprefix)//'counters'//
c     1        rnd_cwhichseed//'.dat',status='unknown')
c      endif
c      call printcnt(iun)
c      close(iun)
c      end
