...
c upper bounding envelope in MINT
      ...
      integer iunit,l
      logical active,btildeflag
      common/storeubc/iunit,active,btildeflag
      save /storeubc/
      data active/.false./
      active=.true.
 ...
 1    if(filetag(l:l).eq.' ') then
     ...
     1        rnd_cwhichseed//'.dat',status='unknown')
      ...
      subroutine storemintupb(ndim,ncell,f)
      implicit none
      include 'nlegborn.h'
      integer ndim,ncell(ndim)
      real * 8 f
      integer iunit
      logical active,btildeflag
      common/storeubc/iunit,active,btildeflag
      ...
      if(active) then
         ...
         if(btildeflag) then
            write(iunit,fmt) (ncell(k),k=1,ndim),f,totborn
         else
 ...
 1       continue
         ...
         do j=1,nlines
            ...
            allf0(j)=f0
         enddo
c this call should return 1 in iret
         call getlinemintupb(filetag,ndim,cells,f,f0,iret)
         status=1
         index=1
      endif
...
c stop updating the rat grid, if satisfactory
 ...
 998  continue
      ...
      if(rnd_cwhichseed.eq.'none') then
         ...
         file=pwgprefix(1:lprefix)//'boundviolations-'//rnd_cwhichseed
     1        //'.dat'
      endif
      call newunit(iun)
