...
!! File generated automatically by autogen.pl from 
!! general precision template qpfiles/genqp/qprecurrenceGbitsfour.f90.

module qprecurrencebitsfour

  use types; use consts_qp
  use qprecurrencebitsone
  use qprecurrencebitstwo
  use qprecurrencebitsthree
  use qpaux_functions
  use qpmemory
  use define_ampl
...
contains
  ...
  function vvww(eW1,kW1,eW2,kW2)  !---- The ZWW or \gammaWW vertex
    complex(qp), intent(in) :: eW1(:), eW2(:)
    complex(qp), intent(in) :: kW1(:), kW2(:)
    complex(qp)	 	    :: vvww(size(eW1))
    !------------------------------------------
    complex(qp):: sk1e2, se1e2, sk2e1, sk1e1, sk2e2
  ...
  function fWW0(sp,p,eW1,kW1,eW2,kW2) !---Two Ws attach directly onto fermions
    complex(qp), intent(in)      :: sp(:), p(:)
    complex(qp), intent(in)    	 :: eW1(:), kW1(:),eW2(:), kW2(:)
    complex(qp)	      		 :: fWW0(size(sp))
    !----------------------------------------
    complex(qp)		:: k1(size(p))
    complex(qp)		:: sp1(size(sp))
    complex(qp)		:: k1sq
    complex(qp)		:: tmp(size(sp))
  ...
  function fVWW0(sp,p,eW,kW) !---WW -> vector boson -> fermion line
    complex(qp), intent(in)      :: sp(:), p(:)
    complex(qp), intent(in)    	 :: eW(:,:), kW(:,:)
    complex(qp)	      		 :: fVWW0(size(sp))
    !----------------------------------------
    complex(qp)		:: k1(size(p))
    complex(qp)		:: e1(size(sp))
  ...
  recursive function fWW_RR(e,k,sp,p,flin, flout, eW,kW,ms,giarray,&
    ...
    complex(qp), intent(in)     :: e(:,:),k(:,:), kW(:,:), eW(:,:)
    complex(qp), intent(in)     :: p(:), sp(:)
    ...
    complex(qp):: e2(size(eW,dim=1)), res(size(sp,dim=1)), tmp(size(sp,dim=1)) 
    complex(qp):: kZ(size(kW,dim=1)),sp2(size(sp)),nm(size(sp2)),  kprop(size(p))
    complex(qp)::  den, den2
    complex(qp):: kgluon(size(k,dim=1))
    character:: flint*3
    logical:: done
    ...
    if (ngluons ==0) then
       ...
       !! sp2 = fW(e,k,sp,p,flint, flout, eW(:,1),kW(:,1),
       !!ng1,giarray,qiarray,WWid(2),pol_int)
  ...
  recursive function fWW_TM(e,k,sp,p,eW1,kW1,eW2,kW2,ms,giarray,&
    ...
    complex(qp), intent(in)	:: e(:,:), k(:,:)
    complex(qp), intent(in)	:: sp(:), p(:)
    complex(qp), intent(in)	:: eW1(:), kW1(:)
    complex(qp), intent(in)	:: eW2(:), kW2(:)
    integer, intent(in)		:: ms
    integer, intent(in), optional :: giarray(:), qiarray(:),iWp,iWm,pol_int  
    !----------------------------------------
    complex(qp)			:: res(size(sp))
    complex(qp)			:: tmp(size(sp))
    complex(qp)			:: k1(size(p))
    complex(qp)			:: k2(size(kW1))
    complex(qp)			:: sp2(size(sp))
    complex(qp)			:: e1(size(e,dim=1))
    complex(qp)			:: k1sq, k2sq
...
!!$   ! C: Recursion. Attach the nth gluon to a current of 2W's and n-1 gluons
  ...
  recursive function gWW_fbf_RR(e,k,sp1,p1,fl1,sp2,p2,fl2,eW,kW,ng1,ng2,&
    ...
    complex(qp), intent(in) :: e(:,:), k(:,:)
    complex(qp), intent(in) :: sp1(:), p1(:), sp2(:), p2(:)
    complex(qp), intent(in) :: eW(:,:),kW(:,:)
    integer, intent(in) ::  ng1,ng2
    character, intent(in) :: fl1*3,fl2*3
    integer, intent(in), optional       :: giarray(:),qiarray(:),WWid(:),pol_int 
    complex(qp)             :: res(size(e,dim=1))
    ! -------------------------------------
    complex(qp) :: eW1(size(ew,dim=1)),eW2(size(ew,dim=1))
    complex(qp) :: kW1(size(ew,dim=1)),kW2(size(ew,dim=1))
    integer :: iWp, iWm 
    eW1 = ew(:,1)
    eW2 = ew(:,2)
    kW1 = kw(:,1)
  ...
  recursive function gWW_fbf_TM(e,k,sp1,p1,fl1,sp2,p2,fl2,&
    ...
    complex(qp)             :: res(size(e,dim=1))
    complex(qp)             :: tmp(size(e,dim=1)),tmp2(size(e,dim=1))
    complex(qp)             :: k1(size(p1))
    complex(qp)             :: k2(size(p1)),k22(size(p1))
    complex(qp)             :: k3(size(p1))
    complex(qp)             :: k4(size(p1))
    complex(qp)             :: kW(size(p1))
    complex(qp)             :: sp3(size(sp1))
    complex(qp)             :: sp4(size(sp1)),sp5(size(sp1))
    complex(qp)             :: e1(size(e,dim=1))
    complex(qp)             :: e2(size(e,dim=1))
    complex(qp)             :: e3(size(e,dim=1))
    complex(qp)  :: k1sq,k2sq,k22sq,k3sq,k4sq
    logical                   :: done 
  ...
  function fWWnogluon(sp,p, eW,kW, giarray,qiarray,WWid, pol_int) result(res)
    complex(qp), intent(in):: kW(:,:), eW(:,:)
    complex(qp), intent(in):: p(:), sp(:)
    integer, intent(in), optional:: giarray(:),qiarray(:),WWid(:),pol_int 
    ! -----------------------------------------------------------------------
    complex(qp):: res(size(sp,dim=1)), tmp(size(sp,dim=1)),e2(size(eW,dim=1))
    complex(qp)::  sp2(size(sp)), kprop(size(p))
    logical done
  ...
  recursive function fWW_bffbf_RR(e,k,sp,p,fll,fl0,&
    ...
    complex(qp), intent(in) :: e(:,:), k(:,:)
    complex(qp), intent(in) :: sp(:,:),p(:,:)
    complex(qp), intent(in) :: eWin(:,:),kWin(:,:)
    integer, intent(in) ::  ng1,ng2,ng3,sw
    character, intent(in) :: fll(:)*3
    character, intent(in) :: fl0*3   ! flavor off-shell f-line
    integer, intent(in), optional       :: giarray(:),qiarray(:),WWid(:),pol_int   ! --------------------------------------------------------------------
    complex(qp)::sp4(size(sp,dim=1)), res(size(sp,dim=1)),tmp(size(sp,dim=1))
    complex(qp):: e2(size(e,dim=1)),sp1(size(sp,dim=1)),sp2(size(sp,dim=1))
    complex(qp)::kW(size(kWin,dim=1), size(kWin,dim=2)), eW(size(eWin,dim=1), size(eWin,dim=2))
    character::flaux*3,fl1*3,fl2*3,fl3*3, fllnew(3)*3
    integer::ngluons,ng4,m,ngL
    integer, parameter::Ndumm = 0
    complex(qp)             :: kdumm(size(k,dim=1),Ndumm),k5sq
    complex(qp)             :: edumm(size(e,dim=1),Ndumm)
    complex(qp)::k2sq,k4sq,k1sq,k4(size(k,dim=1)), k2(size(k,dim=1)), k5(size(k,dim=1))
    complex(qp)::k1(size(k,dim=1)), spnew(size(sp,dim=1),size(sp,dim=2)),pnew(size(p,dim=1),size(p,dim=2))
  ...
  recursive function fWW_bffbf_TM(e,k,sp,p,fll,fl0,&
    ...
    complex(qp), intent(in) :: e(:,:), k(:,:)
    complex(qp), intent(in) :: sp(:,:),p(:,:)
    complex(qp), intent(in) :: eWp(:),kWp(:),eWm(:),kWm(:)
    integer, intent(in) ::  ng1,ng2,ng3,sw
    character, intent(in) :: fll(:)*3
    character, intent(in) :: fl0*3   ! flavor off-shell f-line
    integer, intent(in), optional       :: giarray(:),qiarray(:),iWp,iWm,pol_int 
    ...
    integer, parameter :: Ndumm=0
    complex(qp)             :: res(size(sp,dim=1))
    complex(qp)             :: tmp(size(sp,dim=1))
    complex(qp)             :: k1(size(k,dim=1))
    complex(qp)             :: k2(size(k,dim=1))
    complex(qp)             :: k4(size(k,dim=1))
    complex(qp)             :: sp1(size(sp,dim=1))
    complex(qp)             :: sp2(size(sp,dim=1))
    complex(qp)             :: sp3(size(sp,dim=1))
    complex(qp)             :: sp4(size(sp,dim=1))
    complex(qp)             :: sp5(size(sp,dim=1))
    complex(qp)             :: e1(size(e,dim=1))
    complex(qp)             :: e2(size(e,dim=1))
    complex(qp)             :: kdumm(size(k,dim=1),Ndumm)
    complex(qp)             :: edumm(size(e,dim=1),Ndumm)
    complex(qp)             :: k1sq,k2sq,k4sq
    logical                   :: done 
    ...
    if (ngluon == 0) then 
       ...
       stop 'fWW_bffbf not implemented for ng>0'
    endif
  ...
  recursive function gWW_bff(e,k,sp1,p1,fl1,sp2,p2,fl2,eW,kW,ng1,ng2,&
    ...
    integer :: ngluon,  ng3, ngL 
    complex(qp)             :: res(size(e,dim=1))
    complex(qp)             :: tmp(size(e,dim=1))
    complex(qp)             :: k1(size(p1))
    complex(qp)             :: k2(size(p1))
    complex(qp)             :: k3(size(p1))
    complex(qp)             :: k4(size(p1))
    complex(qp)             :: sp3(size(sp1))
    complex(qp)             :: sp4(size(sp1))
    complex(qp)             :: e1(size(e,dim=1))
    complex(qp)             :: e2(size(e,dim=1))
    complex(qp)             :: e3(size(e,dim=1))
    complex(qp)  :: k1sq,k2sq,k3sq,k4sq
    logical                   :: done 
    character:: flint*3
    ...
    if (ngluon == 0) then 
       ...
       sp4 = fW(e,k,sp1,p1,flint,fl1, eW(:,2), kW(:,2), 0, giarray, &
            &qiarray(2:2), WWid(2), pol_int)
       k2 = p1 + kW(:,2)
       sp4 = ci*(spb2(sp4,k2))/(sc(k2,k2))!-mass2)

       sp3 = bfW(e,k,sp2,p2,flint,fl2,eW(:,1), kW(:,1), 0, giarray, &
            &qiarray(1:1), WWid(1), pol_int)
       k2 = -p2-kW(:,1)
  ...
  recursive function fWW_bffbf_1(e,k,sp,p, fll, fl0, eW, kW, ng1, ng2, ng3,sw,giarray,qiarray,WWid, pol_int) result(res)
    complex(qp), intent(in) :: e(:,:), k(:,:),eW(:,:), kW(:,:)
    complex(qp), intent(in) :: sp(:,:), p(:,:)
    integer, intent(in) ::  ng1,ng2, ng3,sw
    character, intent(in) :: fll(:)*3
    integer, intent(in), optional       :: giarray(:),qiarray(:),WWid(:),pol_int
    ...
    integer::ngluons,ng4
    complex(qp)::res(size(sp,dim=1)), e2(size(e,dim=1)), k2(size(k,dim=1)),k2sq
    complex(qp)::tmp(size(sp,dim=1)), e1(size(e,dim=1))
    complex(qp)::sp1(size(sp,dim=1)), k1(size(k,dim=1)),k1sq
    complex(qp)::sp4(size(sp,dim=1)), k4(size(k,dim=1)),k4sq
    integer, parameter::Ndumm = 0
    complex(qp)             :: kdumm(size(k,dim=1),Ndumm)
    complex(qp)             :: edumm(size(e,dim=1),Ndumm)
...
!!$!! Only done for ngluons = 0 and sw = 2 or 4.
  recursive function bfWW_fbff(e,k,sp,p,fll,fl0,eW,kW,ng1,ng2,ng3,sw,giarray,&
    ...
    complex(qp)::res(size(sp,dim=1)), e2(size(e,dim=1)), k2(size(k,dim=1)),k2sq
    complex(qp)::tmp(size(sp,dim=1))
    complex(qp)::e3(size(e,dim=1)), k3(size(k,dim=1)),k3sq, k4(size(k,dim=1)),k4sq
    complex(qp)::sp1(size(sp,dim=1)),sp2(size(sp,dim=1)), sp3(size(sp,dim=1))
    complex(qp)::k1(size(k,dim=1)), k1sq

    integer, parameter::Ndumm = 0
    complex(qp)             :: kdumm(size(k,dim=1),Ndumm)
    complex(qp)             :: edumm(size(e,dim=1),Ndumm)
    complex(qp):: eww(size(ew,dim=1),size(ew,dim=2))
    complex(qp)::kww(size(kw,dim=1),size(kw,dim=2))
  ...
  recursive function gWW_sbsfbf(e,k,sp,p,fll,eW,kW,ng1,ng2,ng3,ng4,&
    ...
    ! -----------------------------------------------------
    complex(qp)::sp1(size(sp,dim=1)),sp2(size(sp,dim=1)),sp3(size(sp,dim=1))
    complex(qp)::sp4(size(sp,dim=1)),sp5(size(sp,dim=1)),sp6(size(sp,dim=1))
    complex(qp)::sp7(size(sp,dim=1)),e1(size(e,dim=1)),e2(size(e,dim=1))
    complex(qp)::k1(size(k,dim=1)),k2(size(k,dim=1)),k3(size(k,dim=1))
    complex(qp)::k4(size(k,dim=1)),k5(size(k,dim=1)),k6(size(k,dim=1))
    complex(qp)::k7(size(k,dim=1))
    complex(qp)::k1sq,k2sq,k3sq,k4sq,k5sq,k6sq
    integer::Dv,ngluon,ng5
    complex(qp) :: tmp(size(e,dim=1)),res(size(e,dim=1))
    character::fl1*3,fl2*3,fl3*3,fl4*3,flaux*3
    complex(qp)::edumm(4,0),kdumm(4,0)
    logical :: done 
    ...
    if (ngluon == 0) then
       ...
       if ((sw == 2) .or. (sw==4)) then
          sp1 = fWW_bffbf(edumm,kdumm,sp(:,2:4),p(:,2:4),fll(2:4),&
               &fl1,eW,kW,0,0,0,sw-1,giarray,qiarray(2:4),WWid,pol_int)
...
end module qprecurrencebitsfour
