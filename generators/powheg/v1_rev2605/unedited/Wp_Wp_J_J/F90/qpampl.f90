...
!! File generated automatically by autogen.pl from 
!! general precision template qpfiles/genqp/qpampl.f90.

module qpamplitude
  use types; use consts_qp ; use define_ampl
  use qppol_int; use qpaux_functions; use qpvvn
  use qprecurrence;  use qpglobal 
  use qpmemory; use qpcut_utils
  implicit none
  private 
...
contains
  ...
  subroutine givepol(lab,q1,Dv,Ds,tag_pol,Nj,BPOL,POL)
    ...
    complex(qp), intent(in)  :: q1(5) 
    integer, intent(out)       :: Nj
    complex(qp), intent(out) :: BPOL(8,16),POL(8,16)
    ! ---------------------------------------------------
    complex(qp) :: kuks, r12, r14
    complex(qp) :: p(Dv), q14(4),tmp
    complex(qp) :: v(4,4), v1(4),v2(4)
    complex(qp) :: ulist(Ds,8),barulist(Ds,8)
    real(qp)    :: mass 
    integer       :: Nhf,Nhg,i,j
    ! ------------------------------------
  ...
  subroutine ampl(Dv,Ds,Nj1,Nj2,POLI,q1,lab1,tg,ia,ib,BPOLF,q2,lab2,ij,mur)
    ...
    complex(qp), intent(in)  ::  POLI(8,16),BPOLF(8,16)
    complex(qp), intent(in)  ::  q1(5), q2(5)
    integer, intent(in)        ::  ij
    complex(qp), intent(out) ::  mur(10,10)
    ! -----------------------------------------------------------       
    complex(qp) :: res
    complex(qp) :: po(ib-ia+3,Ds),pe(ib-ia+3,Dv)
    complex(qp) :: p(Dv,ib-ia+3),sp(Ds,ib-ia+3)
    complex(qp) :: vg(Dv), vs(Ds),pol_glu(Dv)
    integer       :: ng,nq,nw
  ...
  subroutine calc_ampl(Dv,Ds,ng,nq,nw,sp,p,flampl,vg,vs,iarray_in,pol_int_in)
    ...
    complex(qp), intent(out)   :: vg(Dv),vs(Ds)
    ! -----------------------------------------------
    complex(qp) :: pol_glu(Dv,ng), p_glu(Dv,ng)
    complex(qp) :: pol_q(Ds,nq), p_q(Dv,nq)
    complex(qp) :: p_W(Dv), pol_W(Dv)
    complex(qp) :: p_Wp(Dv), pol_Wp(Dv)
    complex(qp) :: p_Wm(Dv), pol_Wm(Dv)
    complex(qp) :: p_WW(Dv,nw), pol_WW(Dv,nw), holder(Dv,nw)
    ...
    if (nw == 0) then ! first case, no vector boson
       ...
       elseif (((extra_ferm_pair_nf.eqv..true.) .and. (WWqqqq .eqv. .false.)) .and. &
            & ((fl(1) == 'top'.or.fl(1) == 'bot'.or.&
               &fl(1) == 'str').and.nq == 4)) then 
!???
!       elseif ( ( (extra_ferm_pair_nf.eqv..true.) .or. 
!          (ferm_loops_Z_.eqv..true.) ).and. (WWqqqq .eqv. .false.) .and. &
!            & ((fl(1) == 'top'.or.fl(1) == 'bot'.or.&
!               &fl(1) == 'str'.or. fl(1) == 'chr').and.nq == 4)) then 
...
!!$           write(*,* ) 'fl ', fl
!!$           write(*,*) iq
             !      write(*,*) 'fl1 ',fl1(2), fl1(3), case_one
...
!          if (((fl1(2) == 'bot') .or. (fl1(2) == 'top')) .or. &
!               &(.not. WWqqqq))then ! XXXXX
    ...
    elseif (nw ==2) then
      ...
      elseif ((nq == 4) .and. (WWqqqq .eqv. .true.)) then
             ...
             if ((iWW(2) < pos3(1)) .and. (iWW(1) < pos3(1))) then
                ...
                ng2 = pos3(2) - pos3(1) - 1
                ng3 = pos3(3)-pos3(2) -1
             elseif ((iWW(2) < pos3(1)) .and. (iWW(1) > pos3(2)) .and. (iWW(1) < pos3(3))) then
                sw = 2
                ng1 = pos3(1)-3
                ng2 = pos3(2)-pos3(1)-1
                ng3 = pos3(3)-pos3(2)-2
                ! This case is needed for B1.
...
! -------------------------------------------- RRR 
  ...
  subroutine split_quarks_and_gluons(Dv,sp,p,fl,iarray,&
    ...
    complex(qp), intent(out):: pol_glu(:,:),p_glu(:,:)
    complex(qp), intent(out):: pol_q(:,:),p_q(:,:)
  ...
  subroutine split_W_quarks_and_gluons(Dv,sp,p,fl,iarray,&
    ...
    complex(qp), intent(out) :: pol_glu(:,:),p_glu(:,:)
    complex(qp), intent(out) :: pol_q(:,:),p_q(:,:)
    complex(qp), intent(out) :: pol_W(:),p_W(:)
    character, intent(out)     :: fl1(:)*3
    integer, intent(out)       :: pos(:)
    integer, intent(inout)     :: ig,iq 
 ...
 subroutine split_WW_quarks_and_gluons(Dv,sp,p,fl,iarray,&
    ...
    complex(qp), intent(out) :: pol_glu(:,:),p_glu(:,:)
    complex(qp), intent(out) :: pol_q(:,:),p_q(:,:)
    complex(qp), intent(out) :: pol_Wp(:),p_Wp(:)
    complex(qp), intent(out) :: pol_Wm(:),p_Wm(:)
    character, intent(out)     :: fl1(:)*3
    integer, intent(out)       :: pos(:)
    integer, intent(inout)     :: ig,iq 
