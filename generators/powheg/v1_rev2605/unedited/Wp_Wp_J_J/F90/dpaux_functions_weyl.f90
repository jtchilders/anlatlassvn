...
contains
  ...
  subroutine set_pol_ext(pext)
    ...
    complex(dp), intent(in) :: pext(:,:)
    complex(dp) :: pWp(4),pWm(4),pZ(4),eWp(4),eWm(4)
    complex(dp) :: pp(size(pext,dim=1),size(pext,dim=2))
    complex(dp) :: pol_plus(4),pol_minus(4) 
    integer       :: ihel, i,j  
  ...
  subroutine pol_qqZgluons(pext,pl,pa,pl2,pa2,hl,e,outgoing)
    use dpauxiliary_functions 
    integer, intent(in)           :: hl(:)
    complex(dp), intent(in)     :: pext(:,:),pl(:),pa(:),pl2(:),pa2(:)
    logical, intent(in), optional :: outgoing(:)
    complex(dp), intent(out)    :: e(:,:)
    integer :: i 
    complex(dp) :: pWp(4),pWm(4),pZ(4),eWp(4),eWm(4)
...
!--------massive vector boson with decay polarization
  ...
  function pol_decay(p,m,i)
    ...
    integer, intent(in)       :: i
    ! -------------------------------------
    complex(dp) :: pol_decay(4),zl(4),za(4),Ub(4),V(4)
    real(dp) :: p0,px,py,pz,q(4),lcm(4),l(4),a(4)
    real(dp) :: theta
...
!^^^END   
 ...
 logical function quark_flavour(fl)
    ...
    elseif (fl == 'glu' .or. fl == 'wwp' .or. fl == 'wwm' .or. fl == 'www') then 
       ...
       write(*,*) 'fl',fl 
       stop 'dpampl:: quark_flavour: unrecognized flavour' 
    endif 
