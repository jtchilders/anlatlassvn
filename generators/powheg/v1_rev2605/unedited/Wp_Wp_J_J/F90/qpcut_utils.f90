...
module qpcut_utils
  ...
  use qpmatrix_routines; use assertions 
  implicit none 
  private 
...
contains 
  ...
  function deltaM(kv1,kv2,new,ind,newout) 
    complex(qp), intent(in) :: kv1(:,:),kv2(:,:)
    logical, intent(in)     :: new  
    integer, intent(in)     :: ind 
    logical, intent(in), optional     :: newout 
    !-----------------------------------------------------------------
    complex(qp), save :: delta_mat(dimmax,dimmax) != czero 
    complex(qp) :: deltaM
    integer :: i,j,M,D
    logical :: newout_lcl 
  ...
  function deltaMprime(kv1,kv2,new,ind,read_prev,offset) 
    ...
    !-----------------------------------------------------------------
    complex(qp), save :: delta_mat(dimmax,dimmax) != czero 
    complex(qp), save :: delta_mat_store(dimmax,dimmax) != czero 
    complex(qp) :: deltaMprime
    integer :: i,j,M,D
  ...
  function deltavM(kv1,kv2,read_prev,offset) 
    ...
    complex(qp) :: muv(size(kv1,dim=1)),kv2p(size(kv1,dim=1),size(kv2,dim=2)+1)
    complex(qp) :: deltavM(size(kv1,dim=1)) 
  ...
  function Gram(p)
    ...
    complex(qp) :: Gram !, gram2 
    complex(qp) :: delta_mat(size(p,dim=2),size(p,dim=2))
    integer :: i,j,M
  ...
  subroutine compute_pol(p,gv,b,pol)
    complex(qp), intent(in)       :: p(:),gv(:),b(:,:)
    complex(qp), intent(inout) :: pol(:,:)
    ! -----------------------------------------------------------
    complex(qp) :: pa(size(p,dim=1),2) 
  ...
  subroutine compute_ni(p,b,nv,computing_pol)
    complex(qp), intent(in) :: p(:,:),b(:,:)
    complex(qp), intent(inout) :: nv(:,:)
    logical, intent(in), optional :: computing_pol
    ! -------------------------------------------------------------
    complex(qp) :: kv(size(p,dim=1),size(p,dim=2)+size(b,dim=2)),tmp
    integer     :: i,j,k,D,M
  ...
  subroutine compute_vi(ki,vi) 
    complex(qp), intent(in)  :: ki(:,:)
    complex(qp), intent(out) :: vi(:,:)
    complex(qp) :: kiprime(size(ki,dim=1),size(ki,dim=2))
    complex(qp) :: muv(size(ki,dim=1)),tmp
    integer :: i1,i2,i,j 
    logical :: new, newout
  ...
  subroutine compute_bigV(qi,vi,V)
    complex(qp), intent(in)    :: qi(:,:),vi(:,:)
    complex(qp), intent(inout) :: V(:)
    complex(qp) :: di(size(qi,dim=2))!,qisum(size(qi,dim=1))
    integer :: i,im1 
  ...
  function loopmom(V,alphai,ni) 
    complex(qp), intent(in) :: V(:),alphai(:),ni(:,:)
    complex(qp)             :: loopmom(size(V,dim=1)),tmp
    integer :: i
  ...
  function loopmoms(V,alphai,ni) 
    complex(qp), intent(in)    :: V(:),alphai(:),ni(:,:)
    complex(qp)                :: loopmoms(size(V,dim=1))
    integer :: i
