...
module dpmemory 
  ...
  complex(dp), allocatable, public :: ExtCurrent(:,:) 
  logical, allocatable, public :: log_ExtCurrent(:) 

  complex(dp), allocatable, public :: IntCurrent(:,:,:) 
  logical, allocatable, public :: log_IntCurrent(:,:) 

  complex(dp), allocatable, public :: IntQCurrent(:,:,:) 
...
contains 
  ...
  subroutine memory_check(pol_int,res,done,giarray,qiarray,wid1,wid2)
    ...
    ! -- need to change cashing for ferm-loops Z because of possible 
    !    different ordering of gluons wrt fermion pair
