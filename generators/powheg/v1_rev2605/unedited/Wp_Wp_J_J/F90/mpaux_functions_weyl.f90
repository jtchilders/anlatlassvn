...
contains
  ...
  subroutine set_pol_ext(pext)
    ...
    type(mp_complex), intent(in) :: pext(:,:)
    type(mp_complex) :: pWp(4),pWm(4),pZ(4),eWp(4),eWm(4)
    type(mp_complex) :: pp(size(pext,dim=1),size(pext,dim=2))
    type(mp_complex) :: pol_plus(4),pol_minus(4) 
    integer       :: ihel, i,j  
  ...
  subroutine pol_qqWgluons(pext,pl,pa,hl,e,outgoing)
    ...
    type(mp_complex), intent(in)     :: pext(:,:),pl(:),pa(:)
    logical, intent(in), optional :: outgoing(:)
    type(mp_complex), intent(out)    :: e(:,:)
    integer :: i 
  ...
  subroutine pol_qqZgluons(pext,pl,pa,pl2,pa2,hl,e,outgoing)
    use mpauxiliary_functions 
    integer, intent(in)           :: hl(:)
    type(mp_complex), intent(in)     :: pext(:,:),pl(:),pa(:),pl2(:),pa2(:)
    logical, intent(in), optional :: outgoing(:)
    type(mp_complex), intent(out)    :: e(:,:)
    integer :: i 
    type(mp_complex) :: pWp(4),pWm(4),pZ(4),eWp(4),eWm(4)
 ...
 subroutine pol_qqWWgluons(pext,pl,pa,hl,e,outgoing)
    ...
    if (first_time) then 
       write(*,*) '--------> warning new polarization routine' 
       first_time = .false. 
    endif
  ...
  function ubar0(p,i)
    type(mp_complex), intent(in) :: p(4)
    integer, intent(in)       :: i
    ! -------------------------------      
    type(mp_complex) :: ubar0(4)
    type(mp_complex) :: fc, fc2
    type(mp_real)    :: p0,px,py,pz
...
!    if (abs(fc2).gt. tol) then 
  ...
  function v0(p,i)
    ...
    integer, intent(in)       :: i
    ! -------------------------------      
    type(mp_complex) :: v0(4)
    type(mp_complex) :: fc2, fc
    type(mp_real)    :: p0,px,py,pz
...
!^^^END
  ...
  function pol_mless(p,i,outgoing)
    ...
    integer :: pol
    type(mp_real) :: p0,px,py,pz
    type(mp_real) :: pv,ct,st,cphi,sphi
    type(mp_complex) :: pol_mless(4)
...
!--------massive vector boson polarization routine
  ...
  function pol_mass(p,m,i,outgoing)
    ...
    logical, intent(in),optional :: outgoing
    type(mp_complex)             :: pol_mass(4)
    ! -------------------------------------
    type(mp_real) :: p0,px,py,pz, pv
    type(mp_real) :: ct,st,cphi,sphi
    integer :: pol
...
!^^^END   
  ...
  function pol_dk2mom(plepton,antilepton,i,outgoing)
    integer, intent(in) :: i
    integer :: j
    type(mp_complex), intent(in) :: plepton(:),antilepton(:)
    logical, intent(in),optional :: outgoing
    type(mp_complex) :: pol_dk2mom(4),Ub(4),V(4),q(4),qsq
...
!--------massive vector boson with decay polarization
  ...
  function pol_decay(p,m,i)
    ...
    integer, intent(in)       :: i
    ! -------------------------------------
    type(mp_complex) :: pol_decay(4),zl(4),za(4),Ub(4),V(4)
    type(mp_real) :: p0,px,py,pz,q(4),lcm(4),l(4),a(4)
    type(mp_real) :: theta
...
!^^^END   
    ...
    subroutine boost(mass,p1,p_in,p_out)
      type(mp_real), intent(in)  ::  mass,p1(4),p_in(4)
      type(mp_real), intent(out) ::  p_out(4)
      ! ------------------------------------       
      type(mp_real) ::  gam,beta(1:3),bdotp
      integer :: j,k
    ...
    function spb2(sp,v) 
      type(mp_complex), intent(in) :: sp(:),v(:)
      type(mp_complex) :: spb2(size(sp))
      type(mp_complex) :: x0(4,4),xx(4,4),xy(4,4)
      type(mp_complex) :: xz(4,4),x5(4,4)
      type(mp_complex) :: y1,y2,y3,y4,bp,bm,cp,cm
    ...
    function  psp1(sp1,sp2)
      type(mp_complex), intent(in) :: sp1(:)
      type(mp_complex), intent(in) :: sp2(:)
      type(mp_complex) :: psp1
    ...
    function spi5(sp)
      type(mp_complex), intent(in) :: sp(:)
      type(mp_complex) :: spi5(size(sp))
      integer :: i,j,imax, Ds  
 ...
 logical function quark_flavour(fl)
    ...
    elseif (fl == 'glu' .or. fl == 'wwp' .or. fl == 'wwm' .or. fl == 'www') then 
       ...
       write(*,*) 'fl',fl 
       stop 'mpampl:: quark_flavour: unrecognized flavour' 
    endif 
