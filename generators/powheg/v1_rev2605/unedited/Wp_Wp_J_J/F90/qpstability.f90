...
module qpstability 
  ...
  real(qp), public :: abs_smallest_den 
...
contains
    ...
    subroutine check_res_fails(res_v1,res_v2,i)
      complex(qp), intent(in) :: res_v1,res_v2 
      integer :: i 
      logical :: fails 
      ...
      if (fails) then 
         ...
         !write(*,*) (abs((res_v1-res_v2)/res_v1)),(abs(res_v1-res_v2)),(abs(res_v1)),(abs(res_v2))
      end if
      ...
      if (fails) then 
         if (i ==1) then 
            !write(*,*) 'tri:large ltr ZERO?',(res_v1-res_v2)
            rescheck_fails_loose(1) = .true. 
         elseif (i ==2) then 
            !write(*,*) 'tri:small ltr ZERO?',(res_v1-res_v2)
            rescheck_fails_loose(2) = .true. 
         elseif (i ==3) then 
            !write(*,*) 'bub:large ltr ZERO?',(res_v1-res_v2)
            rescheck_fails_loose(3) = .true. 
         elseif (i ==4) then 
            !write(*,*) 'bub:small ltr ZERO?',(res_v1-res_v2)
      ...
      fails = .false. 
      if (abs(res_v2) > sq2tol) then 
          if (abs((res_v1-res_v2)/res_v2) > 0.01_qp) fails = .true. 
       else
          if (abs((res_v1-res_v2)) > 0.01_qp) fails = .true. 
       endif
