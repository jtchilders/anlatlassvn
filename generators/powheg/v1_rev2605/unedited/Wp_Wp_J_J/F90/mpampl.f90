...
!! File generated automatically by autogen.pl from 
!! general precision template mpfiles/genmp/mpampl.f90.
...
module mpamplitude
  ...
  use mprecurrence;  use mpglobal 
  use mpmemory; use mpcut_utils
  implicit none
  private 
...
contains
  ...
  subroutine givepol(lab,q1,Dv,Ds,tag_pol,Nj,BPOL,POL)
    ...
    type(mp_complex), intent(in)  :: q1(5) 
    integer, intent(out)       :: Nj
    type(mp_complex), intent(out) :: BPOL(8,16),POL(8,16)
    ! ---------------------------------------------------
    type(mp_complex) :: kuks, r12, r14
    type(mp_complex) :: p(Dv), q14(4),tmp
    type(mp_complex) :: v(4,4), v1(4),v2(4)
    type(mp_complex) :: ulist(Ds,8),barulist(Ds,8)
    type(mp_real)    :: mass 
    integer       :: Nhf,Nhg,i,j
    ! ------------------------------------
  ...
  subroutine ampl(Dv,Ds,Nj1,Nj2,POLI,q1,lab1,tg,ia,ib,BPOLF,q2,lab2,ij,mur)
    ...
    type(mp_complex), intent(in)  ::  POLI(8,16),BPOLF(8,16)
    type(mp_complex), intent(in)  ::  q1(5), q2(5)
    integer, intent(in)        ::  ij
    type(mp_complex), intent(out) ::  mur(10,10)
    ! -----------------------------------------------------------       
    type(mp_complex) :: res
    type(mp_complex) :: po(ib-ia+3,Ds),pe(ib-ia+3,Dv)
    type(mp_complex) :: p(Dv,ib-ia+3),sp(Ds,ib-ia+3)
    type(mp_complex) :: vg(Dv), vs(Ds),pol_glu(Dv)
    integer       :: ng,nq,nw
  ...
  subroutine calc_ampl(Dv,Ds,ng,nq,nw,sp,p,flampl,vg,vs,iarray_in,pol_int_in)
    ...
    type(mp_complex), intent(out)   :: vg(Dv),vs(Ds)
    ! -----------------------------------------------
    type(mp_complex) :: pol_glu(Dv,ng), p_glu(Dv,ng)
    type(mp_complex) :: pol_q(Ds,nq), p_q(Dv,nq)
    type(mp_complex) :: p_W(Dv), pol_W(Dv)
    type(mp_complex) :: p_Wp(Dv), pol_Wp(Dv)
    type(mp_complex) :: p_Wm(Dv), pol_Wm(Dv)
    type(mp_complex) :: p_WW(Dv,nw), pol_WW(Dv,nw), holder(Dv,nw)
    ...
    if (nw == 0) then ! first case, no vector boson
       ...
       &pol_glu,p_glu,pol_q,p_q,fl1,pos4,ig,iq,giarray,qiarray)
          ...
          elseif (nq == 4 .and. fl(1) /= 'glu' .and. WWqqqq .eqv. .true.) then
             ...
             elseif((case_a1 .eqv. .true.) .or. (case_b1 .eqv. .true.) .or. &
                  &(case_b2 .eqv. .true.) .or. (case_a3 .eqv. .true.)) then
             ...
             elseif (((case_a4 .eqv. .true.) .or. (case_a2 .eqv. .true.)) .and. &
                  &(fl1(2) == 'str' .and. fl1(3) == 'str')) then
    ...
    elseif (nw == 1) then 
       ...
       elseif ((nq == 4) .and. (fl(1) /= 'glu') .and. (WWqqqq .eqv. .true.)) then
          ...
          if (((isstr .eqv. .false.).and. (fl(1) == 'top' .or. fl(1) == 'bot')) &
               &.or. (((case_a1 .eqv. .true.) .or. (case_b1 .eqv. .true.)) .and. &
               &(fl(1) /= 'glu'))) then 
             pos3(1:3) = pos4(2:4) 
...
!-------------------------------------
       ...
       elseif ((fl(1) == 'top' .or. fl(1) == 'bot') .and. nq == 6) then
          ...
          elseif ((iW.gt.pos5(4)).and.(iW.lt.pos5(5))) then 
             ...
             ng5 = pos5(5)-pos5(4)-2
             sw = 3
    !         write(*,*) 'Calling fW_bffbffbf with sw = 3 and flavours ', fl
          endif
    ...
    elseif (nw ==2) then
      ...
      elseif ((nq == 4) .and. (WWqqqq .eqv. .true.)) then
              ...
              if (((isstr .eqv. .false.).and.&
                   ...
                   &.or. (((case_b1 .eqv. .true.) .or. (case_a1 .eqv. .true.)) &
                   &.and. (fl(1) /= 'glu'))) then         
                 pos3(1:3) = pos4(2:4)
             ...
             if ((iWW(2) < pos3(1)) .and. (iWW(1) < pos3(1))) then
                ...
                ng2 = pos3(2) - pos3(1) - 1
                ng3 = pos3(3)-pos3(2) -1
             elseif ((iWW(2) < pos3(1)) .and. (iWW(1) > pos3(2)) .and. &
                  &(iWW(1) < pos3(3))) then
                sw = 2
                ng1 = pos3(1)-3
                ng2 = pos3(2)-pos3(1)-1
             ...
             elseif ((iWW(1) < pos3(1)) .and. (iWW(2) > pos3(2)) .and. &
                  &(iWW(2) <  pos3(3))) then
                sw = 2
                ng1 = pos3(1)-3
                ng2 = pos3(2)-pos3(1)-1
                ...
                ! This is for case B2. The same as above, except for ng's.
             ...
             elseif ((iWW(2) > pos3(2)) .and. (iWW(2) < pos3(3)) .and. &
                  &(iWW(1) >  pos3(2)) .and. (iWW(2) < pos3(3))) then
                sw = 3
                ng1 = pos3(1)-2
                ng2 = pos3(2)-pos3(1)-1
                ng3 = pos3(3)-pos3(2)-3
          ...
          elseif (isstr .eqv. .true. .and. fl1(1) /= 'glu' .and. &
             ...
             elseif  (iWW(1) > pos3(1) .and. iWW(2) < pos3(2)) then
                ...
                p_WW(:,1) = p_WW(:,2)
                p_WW(:,2) = holder(:,1)
             elseif (iWW(1) < pos3(1) .and. iWW(2) > pos3(2) .and. iWW(2) < pos3(3)) then
                sw = 3
                ng1 = pos3(1)-3
                ng2 = pos3(2)-pos3(1)-1
                ng3 = pos3(3)-pos3(2)-2
...
! -------------------------------------------- RRR 
       elseif ((extra_ferm_pair1.eqv. .false.) .and. &
          ...
          call split_WW_quarks_and_gluons(Dv,sp,p,fl,iarray,&
               &pol_glu,p_glu,pol_q,p_q,pol_Wp,p_Wp,pol_Wm,p_Wm,fl1,pos4,ig,iq,iWp,iWm,&
               &giarray,qiarray,wiarray)
  ...
  subroutine split_quarks_and_gluons(Dv,sp,p,fl,iarray,&
    ...
    type(mp_complex), intent(out):: pol_glu(:,:),p_glu(:,:)
    type(mp_complex), intent(out):: pol_q(:,:),p_q(:,:)
  ...
  subroutine split_W_quarks_and_gluons(Dv,sp,p,fl,iarray,&
    ...
    type(mp_complex), intent(out) :: pol_glu(:,:),p_glu(:,:)
    type(mp_complex), intent(out) :: pol_q(:,:),p_q(:,:)
    type(mp_complex), intent(out) :: pol_W(:),p_W(:)
    character, intent(out)     :: fl1(:)*3
    integer, intent(out)       :: pos(:)
    integer, intent(inout)     :: ig,iq 
 ...
 subroutine split_WW_quarks_and_gluons(Dv,sp,p,fl,iarray,&
    ...
    type(mp_complex), intent(out) :: pol_glu(:,:),p_glu(:,:)
    type(mp_complex), intent(out) :: pol_q(:,:),p_q(:,:)
    type(mp_complex), intent(out) :: pol_Wp(:),p_Wp(:)
    type(mp_complex), intent(out) :: pol_Wm(:),p_Wm(:)
    character, intent(out)     :: fl1(:)*3
    integer, intent(out)       :: pos(:)
    integer, intent(inout)     :: ig,iq 
