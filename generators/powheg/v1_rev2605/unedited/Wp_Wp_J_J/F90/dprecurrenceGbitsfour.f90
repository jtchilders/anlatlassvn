...
!! File generated automatically by autogen.pl from 
!! general precision template dpfiles/gendp/dprecurrenceGbitsfour.f90.

module dprecurrencebitsfour

  use types; use consts_dp
  use dprecurrencebitsone
  use dprecurrencebitstwo
  use dprecurrencebitsthree
  use dpaux_functions
  use dpmemory
  use define_ampl
...
contains
  ...
  function vvww(eW1,kW1,eW2,kW2)  !---- The ZWW or \gammaWW vertex
    complex(dp), intent(in) :: eW1(:), eW2(:)
    complex(dp), intent(in) :: kW1(:), kW2(:)
    complex(dp)	 	    :: vvww(size(eW1))
    !------------------------------------------
    complex(dp):: sk1e2, se1e2, sk2e1, sk1e1, sk2e2
  ...
  function fWW0(sp,p,eW1,kW1,eW2,kW2) !---Two Ws attach directly onto fermions
    ...
    complex(dp), intent(in)    	 :: eW1(:), kW1(:),eW2(:), kW2(:)
    complex(dp)	      		 :: fWW0(size(sp))
    !----------------------------------------
    complex(dp)		:: k1(size(p))
    complex(dp)		:: sp1(size(sp))
    complex(dp)		:: k1sq
    complex(dp)		:: tmp(size(sp))
  ...
  function fVWW0(sp,p,eW,kW) !---WW -> vector boson -> fermion line
    complex(dp), intent(in)      :: sp(:), p(:)
    complex(dp), intent(in)    	 :: eW(:,:), kW(:,:)
    complex(dp)	      		 :: fVWW0(size(sp))
    !----------------------------------------
    complex(dp)		:: k1(size(p))
    complex(dp)		:: e1(size(sp))
  ...
  recursive function fWW_RR(e,k,sp,p,flin, flout, eW,kW,ms,giarray,&
    ...
    complex(dp), intent(in)     :: e(:,:),k(:,:), kW(:,:), eW(:,:)
    complex(dp), intent(in)     :: p(:), sp(:)
    ...
    complex(dp):: e2(size(eW,dim=1)), res(size(sp,dim=1)), tmp(size(sp,dim=1)) 
    complex(dp):: kZ(size(kW,dim=1)),sp2(size(sp)),nm(size(sp2)),  kprop(size(p))
    complex(dp)::  den, den2
    complex(dp):: kgluon(size(k,dim=1))
    character:: flint*3
    logical:: done
  ...
  recursive function fWW_TM(e,k,sp,p,eW1,kW1,eW2,kW2,ms,giarray,&
    ...
    complex(dp), intent(in)	:: e(:,:), k(:,:)
    complex(dp), intent(in)	:: sp(:), p(:)
    complex(dp), intent(in)	:: eW1(:), kW1(:)
    complex(dp), intent(in)	:: eW2(:), kW2(:)
    integer, intent(in)		:: ms
    integer, intent(in), optional :: giarray(:), qiarray(:),iWp,iWm,pol_int  
    !----------------------------------------
    complex(dp)			:: res(size(sp))
    complex(dp)			:: tmp(size(sp))
    complex(dp)			:: k1(size(p))
    complex(dp)			:: k2(size(kW1))
    complex(dp)			:: sp2(size(sp))
    complex(dp)			:: e1(size(e,dim=1))
    complex(dp)			:: k1sq, k2sq
  ...
  recursive function bfWW_RR(e,k,sp,p,flin, flout, eW,kW,ms,giarray,qiarray,WWid, pol_int) result(res)
    complex(dp), intent(in)::e(:,:),k(:,:), kW(:,:), eW(:,:)
    complex(dp), intent(in):: p(:), sp(:)
    character, intent(in) :: flin*3, flout*3
    integer, intent(in), optional       :: giarray(:),qiarray(:),WWid(:),pol_int,ms 
...
!!$   ! C: Recursion. Attach the nth gluon to a current of 2W's and n-1 gluons
  ...
  recursive function bfWW_TM(e,k,sp,p,eW1,kW1,eW2,kW2,ms,giarray,&
    ...
    if (present(giarray)) then 
       !if (size(qiarray) /= 1) stop 'bfWW: wrong size qiarray'
       !if (size(e,dim=2) /= size(giarray)) stop 'bfWW: ng= size(giarray)' 
       call memory_check(pol_int,res,done,giarray,qiarray,iWp,iWm)
       if (done) return 
    else
       if (i_warn < max_warn) then 
  ...
  recursive function gWW_fbf_RR(e,k,sp1,p1,fl1,sp2,p2,fl2,eW,kW,ng1,ng2,&
    ...
    complex(dp), intent(in) :: e(:,:), k(:,:)
    complex(dp), intent(in) :: sp1(:), p1(:), sp2(:), p2(:)
    complex(dp), intent(in) :: eW(:,:),kW(:,:)
    integer, intent(in) ::  ng1,ng2
    character, intent(in) :: fl1*3,fl2*3
    integer, intent(in), optional       :: giarray(:),qiarray(:),WWid(:),pol_int 
    complex(dp)             :: res(size(e,dim=1))
    ! -------------------------------------
    complex(dp) :: eW1(size(ew,dim=1)),eW2(size(ew,dim=1))
    complex(dp) :: kW1(size(ew,dim=1)),kW2(size(ew,dim=1))
    integer :: iWp, iWm 
    eW1 = ew(:,1)
    eW2 = ew(:,2)
    kW1 = kw(:,1)
  ...
  recursive function gWW_fbf_TM(e,k,sp1,p1,fl1,sp2,p2,fl2,&
    ...
    integer, intent(in), optional       :: giarray(:),qiarray(:),iWp,iWm,pol_int 
    ! -----------------------------------------------------------------------
    integer             :: m1,m2, ms1a, ms2a,Dv
    integer :: ngluon,  ng3, ngL 
    complex(dp)             :: res(size(e,dim=1))
    complex(dp)             :: tmp(size(e,dim=1)),tmp2(size(e,dim=1))
    complex(dp)             :: k1(size(p1))
    complex(dp)             :: k2(size(p1)),k22(size(p1))
    complex(dp)             :: k3(size(p1))
    complex(dp)             :: k4(size(p1))
    complex(dp)             :: kW(size(p1))
    complex(dp)             :: sp3(size(sp1))
    complex(dp)             :: sp4(size(sp1)),sp5(size(sp1))
    complex(dp)             :: e1(size(e,dim=1))
    complex(dp)             :: e2(size(e,dim=1))
    complex(dp)             :: e3(size(e,dim=1))
    complex(dp)  :: k1sq,k2sq,k22sq,k3sq,k4sq
    logical                   :: done 
  ...
  ! This is the current of fWW (no gluons) with the W's coupling
  ! directly to the fermion line.
  function fWWnogluon(sp,p, eW,kW, giarray,qiarray,WWid, pol_int) result(res)
    complex(dp), intent(in):: kW(:,:), eW(:,:)
    complex(dp), intent(in):: p(:), sp(:)
    integer, intent(in), optional:: giarray(:),qiarray(:),WWid(:),pol_int 
    ! -----------------------------------------------------------------------
    complex(dp):: res(size(sp,dim=1)), tmp(size(sp,dim=1)),e2(size(eW,dim=1))
    complex(dp)::  sp2(size(sp)), kprop(size(p))
    logical done
  ...
  recursive function fWW_bffbf_RR(e,k,sp,p,fll,fl0,&
    ...
    complex(dp), intent(in) :: e(:,:), k(:,:)
    complex(dp), intent(in) :: sp(:,:),p(:,:)
    complex(dp), intent(in) :: eWin(:,:),kWin(:,:)
    integer, intent(in) ::  ng1,ng2,ng3,sw
    character, intent(in) :: fll(:)*3
    character, intent(in) :: fl0*3   ! flavor off-shell f-line
    integer, intent(in), optional       :: giarray(:),qiarray(:),WWid(:),pol_int   ! --------------------------------------------------------------------
    complex(dp)::sp4(size(sp,dim=1)), res(size(sp,dim=1)),tmp(size(sp,dim=1))
    complex(dp):: e2(size(e,dim=1)),sp1(size(sp,dim=1)),sp2(size(sp,dim=1))
    complex(dp)::kW(size(kWin,dim=1), size(kWin,dim=2)), eW(size(eWin,dim=1), size(eWin,dim=2))
    character::flaux*3,fl1*3,fl2*3,fl3*3, fllnew(3)*3
    integer::ngluons,ng4,m,ngL
    integer, parameter::Ndumm = 0
    complex(dp)             :: kdumm(size(k,dim=1),Ndumm),k5sq
    complex(dp)             :: edumm(size(e,dim=1),Ndumm)
    complex(dp)::k2sq,k4sq,k1sq,k4(size(k,dim=1)), k2(size(k,dim=1)), k5(size(k,dim=1))
    complex(dp)::k1(size(k,dim=1)), spnew(size(sp,dim=1),size(sp,dim=2)),pnew(size(p,dim=1),size(p,dim=2))
  ...
  recursive function fWW_bffbf_TM(e,k,sp,p,fll,fl0,&
    ...
    complex(dp), intent(in) :: e(:,:), k(:,:)
    complex(dp), intent(in) :: sp(:,:),p(:,:)
    complex(dp), intent(in) :: eWp(:),kWp(:),eWm(:),kWm(:)
    integer, intent(in) ::  ng1,ng2,ng3,sw
    character, intent(in) :: fll(:)*3
    character, intent(in) :: fl0*3   ! flavor off-shell f-line
    integer, intent(in), optional       :: giarray(:),qiarray(:),iWp,iWm,pol_int 
    ...
    integer, parameter :: Ndumm=0
    complex(dp)             :: res(size(sp,dim=1))
    complex(dp)             :: tmp(size(sp,dim=1))
    complex(dp)             :: k1(size(k,dim=1))
    complex(dp)             :: k2(size(k,dim=1))
    complex(dp)             :: k4(size(k,dim=1))
    complex(dp)             :: sp1(size(sp,dim=1))
    complex(dp)             :: sp2(size(sp,dim=1))
    complex(dp)             :: sp3(size(sp,dim=1))
    complex(dp)             :: sp4(size(sp,dim=1))
    complex(dp)             :: sp5(size(sp,dim=1))
    complex(dp)             :: e1(size(e,dim=1))
    complex(dp)             :: e2(size(e,dim=1))
    complex(dp)             :: kdumm(size(k,dim=1),Ndumm)
    complex(dp)             :: edumm(size(e,dim=1),Ndumm)
    complex(dp)             :: k1sq,k2sq,k4sq
    logical                   :: done 
    ...
    if (ngluon == 0) then 
       ...
       if (sw == 1) then
          ...
          do m = 0,ng2
             ...
             e2 = g_fbf(e(:,ng1+m+1:ngluon),k(:,ng1+m+1:ngluon),sp(:,2),&
                  &p(:,2),fl2,sp(:,3),p(:,3),fl3,&
  ...
  recursive function gWW_bff(e,k,sp1,p1,fl1,sp2,p2,fl2,eW,kW,ng1,ng2,&
    ...
    integer :: ngluon,  ng3, ngL 
    complex(dp)             :: res(size(e,dim=1))
    complex(dp)             :: tmp(size(e,dim=1))
    complex(dp)             :: k1(size(p1))
    complex(dp)             :: k2(size(p1))
    complex(dp)             :: k3(size(p1))
    complex(dp)             :: k4(size(p1))
    complex(dp)             :: sp3(size(sp1))
    complex(dp)             :: sp4(size(sp1))
    complex(dp)             :: e1(size(e,dim=1))
    complex(dp)             :: e2(size(e,dim=1))
    complex(dp)             :: e3(size(e,dim=1))
    complex(dp)  :: k1sq,k2sq,k3sq,k4sq
    logical                   :: done 
    character:: flint*3
  ...
  recursive function fWW_bffbf_1(e,k,sp,p, fll, fl0, eW, kW, ng1, ng2, ng3,sw,giarray,qiarray,WWid, pol_int) result(res)
    complex(dp), intent(in) :: e(:,:), k(:,:),eW(:,:), kW(:,:)
    complex(dp), intent(in) :: sp(:,:), p(:,:)
    integer, intent(in) ::  ng1,ng2, ng3,sw
    character, intent(in) :: fll(:)*3
    integer, intent(in), optional       :: giarray(:),qiarray(:),WWid(:),pol_int
    ...
    integer::ngluons,ng4
    complex(dp)::res(size(sp,dim=1)), e2(size(e,dim=1)), k2(size(k,dim=1)),k2sq
    complex(dp)::tmp(size(sp,dim=1)), e1(size(e,dim=1))
    complex(dp)::sp1(size(sp,dim=1)), k1(size(k,dim=1)),k1sq
    complex(dp)::sp4(size(sp,dim=1)), k4(size(k,dim=1)),k4sq
    integer, parameter::Ndumm = 0
    complex(dp)             :: kdumm(size(k,dim=1),Ndumm)
    complex(dp)             :: edumm(size(e,dim=1),Ndumm)
    ...
    if (ngluons == 0) then
!!$! Only one term for the time being...
       if (fl0 == 'top')  flaux = 'bot'
       if (fl0 == 'bot')  flaux = 'top'
...
!!$!! Only done for ngluons = 0 and sw = 2 or 4.
  recursive function bfWW_fbff(e,k,sp,p,fll,fl0,eW,kW,ng1,ng2,ng3,sw,giarray,qiarray,WWid, pol_int) result(res)
    complex(dp), intent(in) :: e(:,:), k(:,:),eW(:,:), kW(:,:)
    complex(dp), intent(in) :: sp(:,:), p(:,:)
    integer, intent(in) ::  ng1,ng2, ng3,sw
    character, intent(in) :: fll(:)*3
    ...
    complex(dp)::res(size(sp,dim=1)), e2(size(e,dim=1)), k2(size(k,dim=1)),k2sq
    complex(dp)::tmp(size(sp,dim=1))
    complex(dp)::e3(size(e,dim=1)), k3(size(k,dim=1)),k3sq, k4(size(k,dim=1)),k4sq
    complex(dp)::sp1(size(sp,dim=1)),sp2(size(sp,dim=1)), sp3(size(sp,dim=1))
    complex(dp)::k1(size(k,dim=1)), k1sq

    integer, parameter::Ndumm = 0
    complex(dp)             :: kdumm(size(k,dim=1),Ndumm)
    complex(dp)             :: edumm(size(e,dim=1),Ndumm)
    complex(dp):: eww(size(ew,dim=1),size(ew,dim=2))
    complex(dp)::kww(size(kw,dim=1),size(kw,dim=2))
    ...
    if (ngluons == 0) then
       ...
       if (sw == 2) then
          ...
          e2 = gWW_fbf(edumm,kdumm,sp(:,1),p(:,1),fl1,sp(:,2),p(:,2),fl2,eW,kW,0,0,giarray,qiarray(1:2),WWid,pol_int)
  ...
  recursive function gWW_sbsfbf(e,k,sp,p,fll,eW,kW,ng1,ng2,ng3,ng4,&
    ...
    ! -----------------------------------------------------
    complex(dp)::sp1(size(sp,dim=1)),sp2(size(sp,dim=1)),sp3(size(sp,dim=1))
    complex(dp)::sp4(size(sp,dim=1)),sp5(size(sp,dim=1)),sp6(size(sp,dim=1))
    complex(dp)::sp7(size(sp,dim=1)),e1(size(e,dim=1)),e2(size(e,dim=1))
    complex(dp)::k1(size(k,dim=1)),k2(size(k,dim=1)),k3(size(k,dim=1))
    complex(dp)::k4(size(k,dim=1)),k5(size(k,dim=1)),k6(size(k,dim=1))
    complex(dp)::k7(size(k,dim=1))
    complex(dp)::k1sq,k2sq,k3sq,k4sq,k5sq,k6sq
    integer::Dv,ngluon,ng5
    !    real(dp):: mt,mass
    complex(dp) :: tmp(size(e,dim=1)),res(size(e,dim=1))
    character::fl1*3,fl2*3,fl3*3,fl4*3,flaux*3
    complex(dp)::edumm(4,0),kdumm(4,0)
    logical :: done 
    ...
    if (ngluon == 0) then
       ...
       if ((sw == 2) .or. (sw==4)) then
          sp1 = fWW_bffbf(edumm,kdumm,sp(:,2:4),p(:,2:4),fll(2:4),&
               &fl1,eW(:,1),kW(:,1),eW(:,2),kW(:,2),0,0,0,sw-1,giarray,qiarray(2:4),WWid(1),WWid(2),pol_int)
...
end module dprecurrencebitsfour
