...
contains
  ...
  subroutine pamplWpWp(pext,pl,pa,hl,lncuts,N5,N4,N3,tN2,N2,N1,&
       ...
       &outgoing,crossed_gl)
    complex(dp), intent(in)  :: pext(:,:),pl(:,:),pa(:,:)
    ...
    logical:: nodum
    complex(dp), intent(out) :: tree
    real(dp), intent(in)     :: mu
    complex(dp), intent(out) :: div2, div1, div3
    ! -------------------------------------------------
    ...
    !integer, dimension(:,:), allocatable :: Lc3a
    integer :: Lc2a(tN2,2)
    character :: F2a(tN2,2)*3
    integer :: Yc2a(tN2,1)
    complex(dp) :: e(size(pext,dim=1),4)
    complex(dp) :: sp(4,Npoint), p(4,Npoint)
    character :: fl1(Npoint)*3
    integer :: ng,nq,nW
    complex(dp) :: vg(4), vs(4)
    ...
    do i=1,Ncut
       ...
       do j=2,Npoint
          ...
          if (Lab_ex(i,j) == 'top' .or. Lab_ex(i,j) == 'bot' .or. &
               &Lab_ex(i,j) == 'str') then 
             mom(i,j,:) = pext(2,:)
             hel(i,j,:) = e(2,:)          
             identity(i,j) = 2
    ...
    do i=1,xN2
       ...
       if (i2.ne.i1 + 1) then 
          ...
          if (Lab_ex(Ncut,i1).ne.'top'.and.Lab_ex(Ncut,i1).ne.'bot'&
               &.and.Lab_ex(Ncut,i1).ne.'glu' .and. Lab_ex(Ncut,i1) &
               &.ne. 'str') then 
             xxx = xxx + 1
          endif
       endif
  ...
  subroutine  oneloopdiv(mu,p,rdiv2,rdiv1,rdiv3)
    ...
    real(dp), intent(in)     :: mu
    complex(dp), intent(out) :: rdiv2, rdiv1,rdiv3
    ! ---------------------------------------------
    complex(dp) :: ap1(size(p,dim=1)), ap2(size(p,dim=1))
    complex(dp) :: s
    real(dp)    :: mu2
    integer       :: i, Nmax
    ...
    if (case_a1) then
       ...
       if (real(s) < zero) then 
          ...
          rdiv3 = rdiv3 -half*(log(mu2/(-s)))**2
       else
          rdiv1 = rdiv1 -log(mu2/s)-ci*pi 
          rdiv3 = rdiv3 -half*((log(mu2/s)**2-pi**2)-ci*two*log(mu2/s)*pi)
       endif
       ...
       if (real(s) < zero) then 
          rdiv3 = rdiv3  + two/three*log(mu2/(-s))
       else
          rdiv3 = rdiv3  + two/three*(log(mu2/s)+ci*pi)
       endif
    ...
    elseif (case_a2 .or. case_a4) then
       ...
       if (real(s) < zero) then 
          rdiv3 = rdiv3  - three/two*log(mu2/(-s))
       else
          rdiv3 = rdiv3  - three/two*(log(mu2/s)+ci*pi)
       endif
