...
module mpmatrix_routines
  ...
  use types; use consts_mp; use warnings_and_errors
  implicit none
...
contains
  ...
  recursive function mydetc(A) result (det)
    ...
    ! dummy variables
    integer     :: m,n,i
    type(mp_complex) ::  r11,r12,r13,r14,r15,r21,r22,r23,r24,r25
    type(mp_complex) ::  r31,r32,r33,r34,r35,r41,r42,r43,r44,r45
    type(mp_complex) ::  r51,r52,r53,r54,r55
  ...
  function determinant_old(matin) result(det)
    use types; use consts_mp
    type(mp_complex), intent(in) :: matin(:,:)
    integer     :: n,i,j
    type(mp_complex) :: det
    type(mp_complex) :: temp(size(matin,dim=2))
    type(mp_complex) :: mat(size(matin,dim=1),size(matin,dim=2))
    integer :: k,kmax
    type(mp_real) :: maxel 
  ...
  function complementc(mat,i,j) result(Aij)
    use types; use consts_mp
    type(mp_complex), intent(in) :: mat(:,:)
    integer, intent(in)     :: i,j 
    type(mp_complex) :: Aij(size(mat,dim=1)-1,size(mat,dim=2)-1)
