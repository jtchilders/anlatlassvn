...
c to find the upper bounding envelope
            call storemintupb(ndim,ncell,f)
            prod=1
            do kdim=1,ndim
...
c     the returned coordinate vector of the generated point
     ...
     #         ymax(nintervals,ndim),x(ndim)
      ...
      logical savelogical
      external fun,random,strictubound
      integer icalls,mcalls,kdim,kint,nintcurr,iret,ifirst,istep
      if(ndim.gt.ndiminteg) then
         write(*,*) 'Mint: at most ',ndiminteg,' dimensions'
         write(*,*) 'Got ',ndim
         call exit(-1)
...
c twice in the computation of btilde,
c the first time to compute the Born contribution (istep=1), the second
c to compute the full contribution (istep=2)
      if(flg_fastbtlbound) then
         savelogical=flg_bornonly
         flg_bornonly=.true.
         istep=1
      endif
 4    continue
...
c get final value (x and vol not used in this call)
      f=fun(x,vol,2)
      if(flg_fastbtlbound.and.istep.eq.1) then
         flg_bornonly=savelogical
         if(.not.flg_bornonly) then
            fsu = f*strictubound(ncell)
c fsu is an upper bound on the full (not bornonly) f;
c if it fails the hit and miss, f also fails
            if(ub.gt.fsu) goto 10
