     ...
     &     hwidth,wwidth,zwidth,twidth
      double precision pwhg_alphas,esq
...
C     set mur/muf 
      ...
      write(6,*) 'Setup specific MCFM variables for this process'
!      q1=-1d0
!      l1=le
!      r1=re
      nwz=0   
      ...
      if ((vdecaymodeZ.eq.11).or.(vdecaymodeZ.eq.13)
     .    .or.(vdecaymodeZ.eq.15)) then
      q1=-1d0
      l1=le
      r1=re
      elseif ((vdecaymodeZ.eq.12).or.(vdecaymodeZ.eq.14)
     .    .or.(vdecaymodeZ.eq.16)) then
      q1=0d0
      l1=ln
      r1=rn
      else
         stop 'vdecaymodeZ invalid (must be between 11....16)' 
      endif
