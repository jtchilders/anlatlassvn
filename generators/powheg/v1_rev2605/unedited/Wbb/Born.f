...
c     Set MADGRAPH Wmass to the W virtuality
         WMASS=kn_masses(3)
         call SDUB_WMBBB(p,amp2_dub_wmbbb_mad)
         call SUBD_WMBBB(p,amp2_ubd_wmbbb_mad)
         call SDBU_WPBBB(p,amp2_dbu_wpbbb_mad)
         call SUDB_WPBBB(p,amp2_udb_wpbbb_mad)
...
c     W Boson                
      ...
      subroutine pwhg_boost(beta,pin,pout)
      implicit none
...
c return values
      ...
      subroutine LH_pwhg_mom(pin,pout)
      implicit none
      ...
      subroutine assign_mom(pin,pout)
      implicit none
...
c      write(*,*) 'begin V decay'      
      ...
      do i=1,nup
         call LH_pwhg_mom(pup(1,i),p(0,i))
      enddo
...
c     go to CM rest frame                  
      ...
      enddo      
      call mboost(nup,beta_L_CM,betamod_L_CM,p,p)

      call LH_pwhg_flav(flav)
c      write(*,*) flav
c      if (nup.eq.5)  then
c         write(*,*) flav
c         write(*,*) nup
...
c     go to V rest frame: it is the 3rd momentum now                  
      do i=1,3
         beta(i) = -p(i,3)/p(0,3)
      enddo
      call pwhg_boost(beta,p(0,3),pw_rest)
c     
...
c     assign momenta for decay process
      call assign_mom(p(0,1),   p_dec(0,1))
      call assign_mom(p(0,2),   p_dec(0,2))
      call assign_mom(p(0,4),   p_dec(0,5))
      call assign_mom(p(0,5),   p_dec(0,6))
...
c     boost back in the CM frame
      ...
      if (nup.eq.5) then 
         call setborn(p,flav,born_pwhg,bornjk,bmunu)
      elseif (nup.eq.6) then
c     the following will be used later by setreal_Wmdecay
         call assign_mom(p(0,6),p_dec(0,7))
         flav_dec(7)=flav(6)
         call setreal(p,flav,amp2_pwhg)
      else
...
c     neutral lepton momentum
      ...
      enddo
      call pwhg_boost(beta,vdec_rest(0,1),vdec(0,1))
      call pwhg_boost(beta,vdec_rest(0,2),vdec(0,2))

c     assign momenta for decay process
      call assign_mom(vdec(0,1),p_dec(0,3))
      call assign_mom(vdec(0,2),p_dec(0,4))

      if (nup.eq.5) then 
         if (idvecbos.eq.24) then
            call setborn_Wpdecay(p_dec,flav_dec,born_dec)
         else
            call setborn_Wmdecay(p_dec,flav_dec,born_dec)
         endif
         ratio = born_dec/born_pwhg
c     write(*,*) ratio
      elseif (nup.eq.6) then
         if (idvecbos.eq.24) then
            call setreal_Wpdecay(p_dec,flav_dec,amp2_dec)
         else
            call setreal_Wmdecay(p_dec,flav_dec,amp2_dec)
         endif
...
c         ini=.false.
      ...
      if(rat.gt.boundnorm) then
         call  increasecnt('W decay upper bound violations')
         if (verbose) then
     ...
     $           ' sqrt(p2)=',sqrt(pwvirt2)
      ...
      if(boundnorm*random().gt.rat) then
         call increasecnt('W decay vetoed configurations')
         goto 1
      endif
...
c     go back to the lab frame
      ...
      enddo  
      call mboost(2,beta_CM_L,betamod_L_CM,vdec(0,1),p_dec(0,3))      
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=UBD_WMBBB(P ,NHEL(1,IHEL),JC(1))            
      ...
      REAL*8 FUNCTION UBD_WMBBB(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      DO I = 1, NCOLOR
          ...
          ENDDO
          UBD_WMBBB =UBD_WMBBB+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C      CALL GAUGECHECK(JAMP,ZTEMP,EIGEN_VEC,EIGEN_VAL,NCOLOR,NEIGEN) 
      END
c
      SUBROUTINE SDUB_WMBBB(P1,ANS)
C  
C Generated by MadGraph II                                              
C MadGraph StandAlone Version
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=DUB_WMBBB(P ,NHEL(1,IHEL),JC(1))            
               ANS(IPROC)=ANS(IPROC)+T
               IF (T .NE. 0D0 .AND. .NOT.    GOODHEL(IHEL,IPROC)) THEN
                   GOODHEL(IHEL,IPROC)=.TRUE.
               ENDIF
      ...
      REAL*8 FUNCTION DUB_WMBBB(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
C ----------
      ...
      DO I = 1, NCOLOR
          ...
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          DUB_WMBBB =DUB_WMBBB+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C      CALL GAUGECHECK(JAMP,ZTEMP,EIGEN_VEC,EIGEN_VAL,NCOLOR,NEIGEN) 
      END
c
      SUBROUTINE SUDB_WPBBB(P1,ANS)
C  
C Generated by MadGraph II                                              
C MadGraph StandAlone Version
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=UDB_WPBBB(P ,NHEL(1,IHEL),JC(1))            
               ANS(IPROC)=ANS(IPROC)+T
               IF (T .NE. 0D0 .AND. .NOT.    GOODHEL(IHEL,IPROC)) THEN
                   GOODHEL(IHEL,IPROC)=.TRUE.
               ENDIF
      ...
      REAL*8 FUNCTION UDB_WPBBB(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
C ----------
      ...
      DO I = 1, NCOLOR
          ...
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          UDB_WPBBB=UDB_WPBBB+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C      CALL GAUGECHECK(JAMP,ZTEMP,EIGEN_VEC,EIGEN_VAL,NCOLOR,NEIGEN) 
      END
c
      SUBROUTINE SDBU_WPBBB(P1,ANS)
C  
C Generated by MadGraph II                                              
C MadGraph StandAlone Version
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=DBU_WPBBB(P ,NHEL(1,IHEL),JC(1))            
               ANS(IPROC)=ANS(IPROC)+T
               IF (T .NE. 0D0 .AND. .NOT.    GOODHEL(IHEL,IPROC)) THEN
                   GOODHEL(IHEL,IPROC)=.TRUE.
               ENDIF
      ...
      REAL*8 FUNCTION DBU_WPBBB(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
C ----------
      ...
      DO I = 1, NCOLOR
          ...
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          DBU_WPBBB =DBU_WPBBB+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C      CALL GAUGECHECK(JAMP,ZTEMP,EIGEN_VEC,EIGEN_VAL,NCOLOR,NEIGEN) 
      END
