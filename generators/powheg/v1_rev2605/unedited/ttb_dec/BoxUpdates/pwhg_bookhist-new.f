...
c underflow. ep trick to make sure that stuff in lower first bin
c and upper last bin edges are not considered underflow or overflow.
      ep=(xhistarr(2,j)-xhistarr(1,j))/1d4
      if(xval.lt.xhistarr(1,j)-ep) then
         yhistarr(0,j)=yhistarr(0,j)+weight
         nhits(0,j)=nhits(0,j)+1
         return
      else
         do k=1,nbins(j)
            if(k.eq.nbins(j)) then
               ep=(xhistarr(k+1,j)-xhistarr(k,j))/1d4
            else
               ep=0
            endif
            if(xval.lt.xhistarr(k+1,j)+ep) then
