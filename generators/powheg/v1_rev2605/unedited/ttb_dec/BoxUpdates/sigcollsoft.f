...
c         write(*,*) alr,flst_emitter(alr),rc(alr)
      enddo
c      write(*,*) ' ========================= '
      end
      ...
      real * 8 rc(maxalr)
      integer alr,em,kres
      real * 8 kperp(0:3),kperp2,q0,xocsi,x,phi
      real * 8 pem(0:3),pres(0:3)
      real * 8 dotp
      external dotp
      em=kn_emitter
      ...
      do alr=1,flst_nalr
         if(em.eq.flst_emitter(alr)) then
            call collfsralr(alr,kn_csi,xocsi,x,q0,kperp,kperp2,rc(alr))
         else
            rc(alr)=0
         endif
      enddo
