...
c      
      subroutine qqwppqq_vonly(pbar,sign, nlo, L,bos,ans)
      implicit none
c
...
c	... should give ratio ~1 ):
            ...
            call gauge_check0_5(j5eu(0,is1,1),
     &           p43(0),1,psi,p,1,2,wet,wut,0d0)
	      if(bad_gauge_sin) bad_gauge = .true.
              
	    call gauge_check0_5(j5ue(0,is1,1)
...
c              print*,'bad gauge check fraction (pent upper line)',
...
c         lfp(js3) = .false.
	 ...
	 if (lpt) then
            call gauge_check0_5(j5eu(0,is3,2),
     &           p21(0),1,psi,p,3,4,wet,wut,0d0)
	      if(bad_gauge_sin) bad_gauge = .true.

	    call gauge_check0_5(j5ue(0,is3,2),
...
c               print*,'bad gauge check fraction (pent lower line)',
...
c  eps1=v,eps2=j_lower,eps3=v'
      m5ue = s1c(psiwu(1,isig1,2),jqq(0,isig3,2),
     &           .true.,isig1,psiwe(1,isig1,1))
      m5eu = s1c(psiwe(1,isig1,2),jqq(0,isig3,2),
...
c  eps1=v,eps2=j_upper,eps3=v'
      m5ue = s1c(psiwu(1,isig3,4),jqq(0,isig1,1),
     &           .true.,isig3,psiwe(1,isig3,3))
      m5eu = s1c(psiwe(1,isig3,4),jqq(0,isig1,1),
