module     p1_dbarc_hepneg_matrix
   ...
   use p1_dbarc_hepneg_model, only: Nf, NC, sqrt2, init_functions
   use p1_dbarc_hepneg_color, only: TR, CA, CF, numcs, &
     & incolors, init_color
   use p1_dbarc_hepneg_diagramsh0l0, only: amplitude0l0 => amplitude
   use p1_dbarc_hepneg_amplitudeh0, only: samplitudeh0l1 => samplitude, &
        &   finite_renormalisation0 => finite_renormalisation
   use p1_dbarc_hepneg_dipoles, only: insertion_operator
...
contains
               ...
               if(PSP_verbosity .ge. 1) then
                  ...
                  if(spprec2 .le. PSP_chk_threshold2 .and. spprec2 .gt. -10000) then
                     write(42,'(4x,A15,A15,A18,A3)') "<process name='", &
                          &   "p1_dbarc_hepneg", "' problem='sinpole","'/>"
                  else if(abs(kfac) > PSP_chk_kfactor) then
                     write(42,'(4x,A15,A15,A18,A3)') "<process name='", &
                          &   "p1_dbarc_hepneg", "' problem='kfactor","'/>"
                  end if
   ...
   subroutine     samplitudel01(vecs, scale2, amp, rat2, ok, h)
      ...
      use p1_dbarc_hepneg_dipoles, only: pi
      implicit none
      real(ki), dimension(6, 4), intent(in) :: vecs
      real(ki), intent(in) :: scale2
      ...
      case default
         ! not implemented
         print*, "In p1_dbarc_hepneg_matrix:"
   ...
   function     samplitudel0(vecs, h) result(amp)
      use p1_dbarc_hepneg_config, only: logfile
      use p1_dbarc_hepneg_kinematics, only: init_event
      implicit none
      real(ki), dimension(6, 4), intent(in) :: vecs
   ...
   pure subroutine color_correlated_lo(color_vector,res)
      use p1_dbarc_hepneg_color, only: T1T1, &
      & T1T2, &
      & T1T6, &
      & T2T2, &
      & T2T6, &
...
end module p1_dbarc_hepneg_matrix
