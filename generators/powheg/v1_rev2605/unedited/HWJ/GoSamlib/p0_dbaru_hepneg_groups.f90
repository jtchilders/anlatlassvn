module     p0_dbaru_hepneg_groups
   use precision, only: ki_sam => ki
   use madds
   use precision_golem, only: ki_gol => ki
   use tens_rec
   use p0_dbaru_hepneg_config, only: ki , reduction_interoperation, samurai_scalar 
   implicit none
   save
...
function     contract_tensor_coefficients_group_0(coeffs) result(amp)
   ...
   use p0_dbaru_hepneg_config, only: debug_nlo_diagrams, logfile
   use p0_dbaru_hepneg_kinematics, only: k1, k2, k3, k4, k5, k6, es234, es345, es12, es3, es56, es23, es61, &
   & es123, es45, es34
...
!---#] subroutine tear_down_golem95:
end module p0_dbaru_hepneg_groups
