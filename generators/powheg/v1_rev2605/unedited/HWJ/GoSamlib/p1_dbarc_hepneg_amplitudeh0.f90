module    p1_dbarc_hepneg_amplitudeh0
   use p1_dbarc_hepneg_config, only: ki, &
       & reduction_interoperation
   use p1_dbarc_hepneg_color, only: numcs
   use p1_dbarc_hepneg_groups
   use precision, only: ki_sam => ki
   use p1_dbarc_hepneg_samuraih0
   use precision_golem, only: ki_gol => ki
   use p1_dbarc_hepneg_golem95h0
...
!---#[ function finite_renormalisation:
   function     finite_renormalisation(scale2) result(amp)
      use p1_dbarc_hepneg_util, only: square
      use p1_dbarc_hepneg_color, only: CF, CA
      use p1_dbarc_hepneg_kinematics, only: &
      & num_light_quarks, num_gluons
      use p1_dbarc_hepneg_diagramsh0l0, only: amplitudel0 => amplitude
      implicit none
   ...
   function     samplitude(scale2,ok,rational2,opt_amp0,opt_perm)
      use p1_dbarc_hepneg_config, only: include_eps_terms, include_eps2_terms, &
      & logfile, debug_nlo_diagrams
      use p1_dbarc_hepneg_globalsl1, only: amp0,perm, use_perm, epspow
      use p1_dbarc_hepneg_globalsh0, &
     & only: init_lo, rat2
      use p1_dbarc_hepneg_abbrevd70h0, only: init_abbrevd70 => init_abbrev
      use p1_dbarc_hepneg_abbrevd61h0, only: init_abbrevd61 => init_abbrev
      use p1_dbarc_hepneg_abbrevd34h0, only: init_abbrevd34 => init_abbrev
      use p1_dbarc_hepneg_abbrevd40h0, only: init_abbrevd40 => init_abbrev
      use p1_dbarc_hepneg_abbrevd60h0, only: init_abbrevd60 => init_abbrev
      use p1_dbarc_hepneg_abbrevd92h0, only: init_abbrevd92 => init_abbrev
      use p1_dbarc_hepneg_abbrevd30h0, only: init_abbrevd30 => init_abbrev
      use p1_dbarc_hepneg_abbrevd37h0, only: init_abbrevd37 => init_abbrev
      use p1_dbarc_hepneg_abbrevd59h0, only: init_abbrevd59 => init_abbrev
      use p1_dbarc_hepneg_abbrevd94h0, only: init_abbrevd94 => init_abbrev
      use p1_dbarc_hepneg_diagramsh0l0, only: amplitudel0 => amplitude
      use p1_dbarc_hepneg_groups
      implicit none
      real(ki), intent(in) :: scale2
...
subroutine     evaluate_group0(scale2,samplitude,ok)
   use p1_dbarc_hepneg_config, only: &
      & logfile, debug_nlo_diagrams
   use p1_dbarc_hepneg_globalsl1, only: epspow
   use parametre, only: mu2_scale_par
   use form_factor_type, only: form_factor
   use p1_dbarc_hepneg_golem95h0, only: reconstruct_golem95 => reconstruct_group
   use p1_dbarc_hepneg_groups, only: contract_golem95, &
      & global_coeffs => coeffs_group0, &
      & reduce_numetens => reduce_numetens_group0
   use p1_dbarc_hepneg_samuraih0, only: samurai_reduce => reduce_group0
...
subroutine     evaluate_group1(scale2,samplitude,ok)
   ...
   use p1_dbarc_hepneg_samuraih0, only: samurai_reduce => reduce_group1
   use options, only: samurai_out => iout
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
...
subroutine     evaluate_group2(scale2,samplitude,ok)
   ...
   use p1_dbarc_hepneg_samuraih0, only: samurai_reduce => reduce_group2
   use options, only: samurai_out => iout
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
...
subroutine     evaluate_group3(scale2,samplitude,ok)
   ...
   use p1_dbarc_hepneg_samuraih0, only: samurai_reduce => reduce_group3
   use options, only: samurai_out => iout
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
...
!---#] subroutine evaluate_group3:
end module p1_dbarc_hepneg_amplitudeh0
