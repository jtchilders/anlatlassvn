...
!---#[ function finite_renormalisation:
   function     finite_renormalisation(scale2) result(amp)
      ...
      use p11_csbar_hepneg_color, only: CF, CA
   ...
   function     samplitude(scale2,ok,rational2,opt_amp0,opt_perm)
      use p11_csbar_hepneg_config, only: include_eps_terms, include_eps2_terms, &
...
subroutine     evaluate_group0(scale2,samplitude,ok)
   ...
   use p11_csbar_hepneg_golem95h0, only: reconstruct_golem95 => reconstruct_group
   use p11_csbar_hepneg_groups, only: contract_golem95, &
      & global_coeffs => coeffs_group0, &
      & reduce_numetens => reduce_numetens_group0
   use p11_csbar_hepneg_samuraih0, only: samurai_reduce => reduce_group0
...
subroutine     evaluate_group1(scale2,samplitude,ok)
   ...
   use p11_csbar_hepneg_samuraih0, only: samurai_reduce => reduce_group1
   use options, only: samurai_out => iout
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
...
subroutine     evaluate_group2(scale2,samplitude,ok)
   ...
   use p11_csbar_hepneg_samuraih0, only: samurai_reduce => reduce_group2
   use options, only: samurai_out => iout
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
...
subroutine     evaluate_group3(scale2,samplitude,ok)
   ...
   use p11_csbar_hepneg_samuraih0, only: samurai_reduce => reduce_group3
   use options, only: samurai_out => iout
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
...
!---#] subroutine evaluate_group3:
end module p11_csbar_hepneg_amplitudeh0
