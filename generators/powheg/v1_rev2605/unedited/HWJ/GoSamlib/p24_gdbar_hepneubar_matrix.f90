module p24_gdbar_hepneubar_matrix
...
contains
   ...
   pure function prefactor()
      ...
      use p0_dbaru_hepneg_model, only: NC
      use p0_dbaru_hepneg_kinematics, only: &
      & in_helicities, symmetry_factor
      use p0_dbaru_hepneg_color, only: incolors
      implicit none
   ...
   subroutine     initgolem(is_first)
      use p0_dbaru_hepneg_matrix, only: orig_initgolem => initgolem
      implicit none
   ...
   subroutine     exitgolem(is_last)
      use p0_dbaru_hepneg_matrix, only: orig_exitgolem => exitgolem
      implicit none
   ...
   subroutine     samplitude(vecs, scale2, amp, ok, h)
      use p0_dbaru_hepneg_matrix, only: orig_sub => samplitude
      implicit none
   ...
   function     samplitudel0(vecs, h) result(amp)
      use p0_dbaru_hepneg_matrix, only: orig_func => samplitudel0
      implicit none
      real(ki), dimension(6, 4), intent(in) :: vecs
   ...
   function     samplitudel1(vecs,scale2,ok,rat2,h) result(amp)
      use p0_dbaru_hepneg_matrix, only: orig_func => samplitudel1
      implicit none
      real(ki), dimension(6, 4), intent(in) :: vecs
...
end module p24_gdbar_hepneubar_matrix
