...
c     writing golem_initialize subroutine
     ...
     $           phys_par_names(i+1)(1:len_trim(phys_par_names(i+1)))
      ...
      write(outcoup,4) "call OLP_Start(path,ioerr)"
     ...
     $     " ' reports an error.'"
      ...
      open(unit=inproc,status='old',file='../init_processes.f')
      open(unit=outproc,file='init_processes_new.f')
      found=.false.
      do i=1,maxnumlines
         read(inproc,'(a)',err=888,end=999) stringa
         if (stringa(1:31).eq.'      subroutine init_processes') 
     $   then
            found=.true.
         endif
         if (found.and.stringa(1:26).eq.'      call gosam_flst_born')
     $   then
            stringa(1:26)='c     call gosam_flst_born'
c            found=.false.
         endif
         write(outproc,1) stringa(1:len_trim(stringa))
      enddo
 888  continue
 999  continue
      if (.not.found) then
         write(*,*) "************************************************"
         write(*,*) "Have NOT found the string 'call gosam_flst_born'"
         write(*,*) "Check that this string is commented in the code!"
         write(*,*) "************************************************"
      endif
      close(inproc)
      close(outproc)
