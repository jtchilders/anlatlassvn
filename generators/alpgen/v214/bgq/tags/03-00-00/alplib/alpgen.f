...
#ifdef USE_MPI
      include 'mpif.h'
      integer rank,mpiworldsize,ierror,tag,status(MPI_STATUS_SIZE)
      real timestamp
      call cpu_time(timestamp)
      call MPI_INIT(ierror)
      call MPI_COMM_SIZE(MPI_COMM_WORLD, mpiworldsize, ierror)
      call MPI_COMM_RANK(MPI_COMM_WORLD, rank, ierror)
      mpirank = rank
      call MPI_FINALIZE(ierror)
      write(6,*) 'MPI finalized: rank ', mpirank,
     $     ' world size = ',mpiworldsize
...
c print rank and time
      ...
      write(6,*) ' Rank: ',rankstring,' Date/Time: ',date,' ',time
...
c input from file
      ...
      open(9,file=input_filename,action='read',iostat=input_file_status)
     ...
     $ '   options and defaults, scale choices, PDF, etc., then stop'
      ...
      if(imode.eq.0.and.mpiworldsize.gt.1) then
         ...
         write(6,*) '  run witn only one rank '
         call exit(-1)
      endif
#endif
c
...
c     
 1    write(6,*) ' '
#ifdef USE_MPI
 2    read(9,*,end=3,err=10) chparam,chvalue
#else
...
c Then read in decay parameters specific to the imode=2 run:
 15   write(6,*) ' '
#ifdef USE_MPI
 20   read(9,*,end=200,err=100) chparam,chvalue
#else
...
c-    the a-priori weights are read from unit= nrd
      ...
      if (nrd.ne.0) then
         ...
         if(filestat /= 0) then
            write(6,*) 'Error reading file file in grid1R subroutine'
         endif
      endif
#else
...
c so we redirect this to the stdout instead
     ...
     $        ,tmpstr(1:l)
      ...
      elseif(imode.eq.2) then
         call alustc(fname_mpirank,'_unw.par',tmpstr)
         l=aluisl(tmpstr)
         write(6,*) 'Parameters and results written to ',tmpstr(1:l)
         call alugun(niosta)
#ifdef USE_MPI
         open(unit=niosta,file=tmpstr,status='unknown',iostat=filestat)
         if(filestat /= 0) then
            write(6,*) 'Error opening file: ',tmpstr
            call exit(-1)
         endif
