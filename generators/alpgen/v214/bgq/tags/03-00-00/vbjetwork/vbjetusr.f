...
*
      do i=1,njets
        call mbook(i,'pt jet',ptbin,0e0,ptmax)
      enddo
      end
...
c
c plot inclusive lepton spectra from Z decays
      do i=1,nz
        if(abs(zfl(i)).eq.11) then
          ptlep=sqrt(idec(1,1,i)**2+idec(2,1,i)**2)
          call mfill(1,real(ptlep),rwgt)
          ptlep=sqrt(idec(1,2,i)**2+idec(2,2,i)**2)
          call mfill(1,real(ptlep),rwgt)
        elseif(abs(zfl(i)).eq.13) then
          ptlep=sqrt(idec(1,1,i)**2+idec(2,1,i)**2)
          call mfill(2,real(ptlep),rwgt)
          ptlep=sqrt(idec(1,2,i)**2+idec(2,2,i)**2)
          call mfill(2,real(ptlep),rwgt)
        elseif(abs(zfl(i)).eq.15) then
          ptlep=sqrt(idec(1,1,i)**2+idec(2,1,i)**2)
          call mfill(3,real(ptlep),rwgt)
          ptlep=sqrt(idec(1,2,i)**2+idec(2,2,i)**2)
          call mfill(3,real(ptlep),rwgt)
        endif
      enddo
c plot inclusive fermion  spectra from W decays
      do i=1,nw
        iw=nz+i
        ptlep=sqrt(idec(1,1,iw)**2+idec(2,1,iw)**2)
        call mfill(4,real(ptlep),rwgt)
        ptlep=sqrt(idec(1,2,iw)**2+idec(2,2,iw)**2)
        call mfill(4,real(ptlep),rwgt)
      enddo
c plot et jets
      do i=1,njets
        ptj=sqrt(p(1,2+i)**2+p(2,2+i)**2)
        call mfill(10+i,real(ptj),rwgt)
      enddo
      end
