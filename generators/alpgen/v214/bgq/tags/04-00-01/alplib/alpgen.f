...
#ifdef USE_MPI
      include 'mpif.h'
      integer mpirank,mpiworldrank,retval
      character*8 rankstring
      common/mpi/mpirank,mpiworldrank,rankstring
      real timestamp
#endif
...
c so we redirect this to the stdout instead
      ...
      if(imode.eq.1) then
#ifdef USE_BGQ_RAMDISK
         call alustc('/dev/persist/'//fname_mpirank,'.par',tmpstr)
#else
     ...
     $        ,tmpstr(1:l)
#ifdef USE_MPI
#ifdef USE_BGQ_RAMDISK
         call alustc('/dev/persist/'//fname_mpirank,'.wgt',tmpstr)
#else
      ...
      elseif(imode.eq.2) then
#ifdef USE_BGQ_RAMDISK
         call alustc('/dev/persist/'//fname_mpirank,'_unw.par',tmpstr)
#else
...
#ifdef USE_BGQ_RAMDISK
         call alustc('/dev/persist/'//fname_mpirank,'.unw',tmpstr)
#else
