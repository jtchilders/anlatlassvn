...
C
          ...
          real*8 gstrong      !SU(3) gauge coupling
...
c
          ...
          init=1
c     gauge sector
          massgaubosons(1)=apar(1)   !M_Z  REAL
          widthgaubosons(1)=apar(2)  !Width(Z)  REAL
          massgaubosons(2)=apar(3)   !M_W  REAL
          widthgaubosons(2)=apar(4)  !Width(W)  REAL
          masshiggs(1)=apar(5)   !M_H  REAL
          widthhiggs(1)=apar(6)  !Width(H)  REAL
          massgaubosons(3)=massgaubosons(2)    !M_W  REAL
          widthgaubosons(3)=widthgaubosons(2)  !Width(W)  REAL

c     quark masses
          massfermion(1,1)=apar(12)     !u mass                     REAL
          massfermion(1,2)=apar(11)     !d mass                     REAL
          massfermion(2,1)=apar(14)     !c mass                     REAL
          massfermion(2,2)=apar(13)     !s mass                     REAL
          massfermion(3,1)=apar(16)     !t mass                     REAL
          massfermion(3,2)=apar(15)     !b mass                     REAL
c
c lepton masses
          massfermion(1,3)=apar(22)     !nue mass                   REAL
          massfermion(1,4)=apar(21)     !e mass                     REAL
          massfermion(2,3)=apar(24)     !numu mass                   REAL
          massfermion(2,4)=apar(23)     !mu mass                     REAL
          massfermion(3,3)=apar(26)     !nutau mass                   REAL
          massfermion(3,4)=apar(25)     !tau mass                     REAL
c
c couplings
          gsu2l=apar(51)       !Su2 coupling constant g2   REAL def=0.6517d0 
          ctetaw=apar(53)
          stetaw=apar(54)
          gbar=apar(55)
C
...
C
          ...
          vectorial(2,1,1,1,2)=-gsu2l/2.d0/sqrt(2.d0)  !W+ u d  vectorial coupling 
          vectorial(2,1,3,1,4)=vectorial(2,1,1,1,2)    !W+ nu e  vectorial coupling 
          vectorial(3,1,2,1,1)=vectorial(2,1,1,1,2)    !W- d u  vectorial coupling 
          vectorial(3,1,4,1,3)=vectorial(2,1,1,1,2)    !W- e nu  vectorial coupling 
          assial(2,1,1,1,2)=-vectorial(2,1,1,1,2)      !W+ u d  assial coupling 
          assial(2,1,3,1,4)=-vectorial(2,1,1,1,2)      !W+ nu e  assial coupling 
          assial(3,1,2,1,1)=-vectorial(2,1,1,1,2)      !W- d u  assial coupling   
          assial(3,1,4,1,3)=-vectorial(2,1,1,1,2)      !W- e nu  assial coupling 
          vectorial(2,2,1,2,2)=vectorial(2,1,1,1,2)    !W+ c s  vectorial coupling 
          vectorial(3,2,2,2,1)=vectorial(2,1,1,1,2)    !W- s c  vectorial coupling 
          assial(2,2,1,2,2)=-vectorial(2,1,1,1,2)      !W+ c s  assial coupling   
          assial(3,2,2,2,1)=-vectorial(2,1,1,1,2)      !W- s c  assial coupling 
          vectorial(2,3,1,3,2)=vectorial(2,1,1,1,2)    !W+ t b  vectorial coupling 
          vectorial(3,3,2,3,1)=vectorial(2,1,1,1,2)    !W- b t  vectorial coupling 
          assial(2,3,1,3,2)=-vectorial(2,1,1,1,2)      !W+ t b  assial coupling   
          assial(3,3,2,3,1)=-vectorial(2,1,1,1,2)      !W- b t  assial coupling 
c
          vectorial(1,1,1,1,1)=-gbar*(1.d0-stetaw**2*8.d0/3.d0)/4.d0*re  !Z \bar u u     V coupling
          vectorial(1,1,2,1,2)=-gbar*(-1.d0+stetaw**2*4.d0/3.d0)/4.d0*re  !Z \bar d d     V coupling
          vectorial(10,1,1,1,1)=-gbar*ctetaw*stetaw*2.d0/3.d0*re  !A \bar u u     V coupling
          vectorial(10,1,2,1,2)=gbar*ctetaw*stetaw/3.d0*re  !A \bar d d     V coupling
          vectorial(1,2,1,2,1)=-gbar*(1.d0-stetaw**2*8.d0/3.d0)/4.d0*re  !Z \bar c c      V coupling
          vectorial(1,2,2,2,2)=-gbar*(-1.d0+stetaw**2*4.d0/3.d0)/4.d0*re  !Z \bar s s     V coupling
          vectorial(10,2,1,2,1)=-gbar*ctetaw*stetaw*2.d0/3.d0*re  !A \bar c c     V coupling
          vectorial(10,2,2,2,2)=gbar*ctetaw*stetaw/3.d0*re  !A \bar s s     V coupling
          vectorial(1,3,1,3,1)=-gbar*(1.d0-stetaw**2*8.d0/3.d0)/4.d0*re  !Z \bar t t      V coupling
          vectorial(1,3,2,3,2)=-gbar*(-1.d0+stetaw**2*4.d0/3.d0)/4.d0*re  !Z \bar b b     V coupling
          vectorial(10,3,1,3,1)=-gbar*ctetaw*stetaw*2.d0/3.d0*re  !A \bar t t     V coupling
          vectorial(10,3,2,3,2)=gbar*ctetaw*stetaw/3.d0*re  !A \bar b b     V coupling
          vectorial(1,1,3,1,3)=-gbar*(1.d0)/4.d0*re  !Z \bar \nu_e \nu_e     V coupling
          vectorial(1,1,4,1,4)=-gbar*(-1.d0+stetaw**2*4.d0)/4.d0*re  !Z \bar  e e  V
...
c
          higgsgauge(1,1,1)=-gbar*massgaubosons(1)/2.d0*re
          higgsgauge(1,2,3)=-gsu2l*massgaubosons(2)*re
          higgsgauge(3,1,1)=-gbar**2/8.d0*re
          higgsgauge(3,2,3)=-gsu2l**2/4.d0*re
c
          selfhiggs(1)=masshiggs(1)**2.*gbar/4.d0/massgaubosons(1)       !h h h  coupling
          selfhiggs(2)=1.d0                                              !Y_h h h coupling
          selfhiggs(3)=masshiggs(1)**2.*gbar**2/massgaubosons(1)**2/32.  !X_h h h coupling
c
          higgsvev=2.d0*massgaubosons(1)/gbar
          yukawa(3,1,3,1)=massfermion(3,1)/higgsvev    !*sqrt(2.d0)               !top yukawa
          yukawa(3,2,3,2)=massfermion(3,2)/higgsvev    !*sqrt(2.d0)               !bottom yukawa
c
          return
          end
...
C
          ...
          vectorial(1,2,2,2,2)=-gbar*(-1.d0+stetaw**2*4.d0/3.d0)/4.d0*re  !Z \bar s s     V coupling
          vectorial(1,1,2,1,2)=-gbar*(-1.d0+stetaw**2*4.d0/3.d0)/4.d0*re  !Z \bar d d     V coupling
          vectorial(1,3,3,3,3)=-gbar*(1.d0)/4.d0*re  !Z \bar \nu_\tau \nu_\tau     V coupling
          vectorial(1,2,3,2,3)=-gbar*(1.d0)/4.d0*re  !Z \bar \nu_\mu \nu_\mu     V coupling
          vectorial(1,1,3,1,3)=-gbar*(1.d0)/4.d0*re  !Z \bar \nu_e \nu_e     V coupling
          vectorial(1,3,4,3,4)=-gbar*(-1.d0+stetaw**2*4.d0)/4.d0*re  !Z \bar \tau \tau    V 
          vectorial(1,2,4,2,4)=-gbar*(-1.d0+stetaw**2*4.d0)/4.d0*re  !Z \bar \mu \mu     V 
          vectorial(1,1,4,1,4)=-gbar*(-1.d0+stetaw**2*4.d0)/4.d0*re  !Z \bar  e e  V 
          vectorial(10,3,1,3,1)=-gbar*ctetaw*stetaw*2.d0/3.d0*re  !A \bar t t     V coupling
          vectorial(10,2,1,2,1)=-gbar*ctetaw*stetaw*2.d0/3.d0*re  !A \bar c c     V coupling
          vectorial(10,1,1,1,1)=-gbar*ctetaw*stetaw*2.d0/3.d0*re  !A \bar u u     V coupling
...
C
          vectorial(2,3,1,3,2)=-gsu2l/sqrt(2.d0)/2.d0*vtb  !W^+ \bar t b     V coupling
          vectorial(3,3,2,3,1)=-gsu2l/sqrt(2.d0)/2.d0*vbt  !W^- \bar b t     V coupling
          vectorial(2,3,1,2,2)=-gsu2l/sqrt(2.d0)/2.d0*vts  !W^+ \bar t s     V coupling
          vectorial(3,2,2,3,1)=-gsu2l/sqrt(2.d0)/2.d0*vst  !W^- \bar s t     V coupling
          vectorial(2,3,1,1,2)=-gsu2l/sqrt(2.d0)/2.d0*vtd  !W^+ \bar t d     V coupling
          vectorial(3,1,2,3,1)=-gsu2l/sqrt(2.d0)/2.d0*vdt  !W^- \bar d t     V coupling
C
          vectorial(2,2,1,3,2)=-gsu2l/sqrt(2.d0)/2.d0*vcb  !W^+ \bar c b     V coupling
          vectorial(3,3,2,2,1)=-gsu2l/sqrt(2.d0)/2.d0*vbc  !W^- \bar b c     V coupling
          vectorial(2,2,1,2,2)=-gsu2l/sqrt(2.d0)/2.d0*vcs  !W^+ \bar c s     V coupling
          vectorial(3,2,2,2,1)=-gsu2l/sqrt(2.d0)/2.d0*vsc  !W^- \bar s c     V coupling
          vectorial(2,2,1,1,2)=-gsu2l/sqrt(2.d0)/2.d0*vcd  !W^+ \bar c d     V coupling
          vectorial(3,1,2,2,1)=-gsu2l/sqrt(2.d0)/2.d0*vdc  !W^- \bar d c     V coupling
C
          vectorial(2,1,1,3,2)=-gsu2l/sqrt(2.d0)/2.d0*vub  !W^+ \bar u b     V coupling
          vectorial(3,3,2,1,1)=-gsu2l/sqrt(2.d0)/2.d0*vbu  !W^- \bar b u     V coupling
          vectorial(2,1,1,2,2)=-gsu2l/sqrt(2.d0)/2.d0*vus  !W^+ \bar u b     V coupling
          vectorial(3,2,2,1,1)=-gsu2l/sqrt(2.d0)/2.d0*vsu  !W^- \bar b u     V coupling
          vectorial(2,1,1,1,2)=-gsu2l/sqrt(2.d0)/2.d0*vud  !W^+ \bar u b     V coupling
          vectorial(3,1,2,1,1)=-gsu2l/sqrt(2.d0)/2.d0*vdu  !W^- \bar b u     V coupling
C
          vectorial(2,3,3,3,4)=-gsu2l/sqrt(2.d0)/2.d0*re  !W^+ \bar \nu_\tau \tau    V coupling
          vectorial(3,3,4,3,3)=-gsu2l/sqrt(2.d0)/2.d0*re !W^+ \bar \tau \nu_\tau     V coupling
C
          vectorial(2,2,3,2,4)=-gsu2l/sqrt(2.d0)/2.d0*re  !W^+ \bar \nu_\mu \mu    V coupling
          vectorial(3,2,4,2,3)=-gsu2l/sqrt(2.d0)/2.d0*re  !W^+ \bar \mu \nu_\mu     V coupling
C
          vectorial(2,1,3,1,4)=-gsu2l/sqrt(2.d0)/2.d0*re  !W^+ \bar \nu_e e  V coupling
          vectorial(3,1,4,1,3)=-gsu2l/sqrt(2.d0)/2.d0*re  !W^+ \bar e \nu_e  V coupling
C
...
C
          assial(2,3,3,3,4)=gsu2l/sqrt(2.d0)/2.d0*re  !W^+ \bar \nu_\tau \tau  A coupling
          assial(3,3,4,3,3)=gsu2l/sqrt(2.d0)/2.d0*re  !W^- \bar \tau \nu_\tau  A coupling
C
          assial(2,2,3,2,4)=gsu2l/sqrt(2.d0)/2.d0*re !W^+ \bar \nu_\mu \mu    A coupling
          assial(3,2,4,2,3)=gsu2l/sqrt(2.d0)/2.d0 *re !W^- \bar \mu \nu_\mu    A coupling
C
          assial(2,1,3,1,4)=gsu2l/sqrt(2.d0)/2.d0*re  !W^+ \bar \nu_e e        A coupling
          assial(3,1,4,1,3)=gsu2l/sqrt(2.d0)/2.d0*re  !W^- \bar e \nu_e        A coupling
C
