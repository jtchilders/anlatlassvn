...
c initialize output parameters
      ...
      data istart/0/
c      write(6,*)" test 1" , cutkin(7), cutkin(8)
c      write(6,*)" test 2" , ptj1min,ptj1max
      if(istart.eq.0) then
         istart=1
c         ptj1min=cutkin(7)
c         ptj1max=cutkin(8)
         print *,"Gend",ptj1min,ptj1max
      endif

c calculate hr
      pthr= sqrt((p(1,njets+3)+p(1,njets+4))**2+
