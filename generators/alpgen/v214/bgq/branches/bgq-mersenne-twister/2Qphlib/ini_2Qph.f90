...
!!$                                                       gluon,gluon,gluon /)       
  tabint(1:64,1)=(/21,qupl,qupbrl,gluon, qupr,qupbrr,gluon, qdr,qdbrr,gluon, &
                      qdl,qdbrl,gluon,qcr,qcbrr,gluon,qcl,qcbrl,gluon,       &
                      qsr,qsbrr,gluon,qsl,qsbrl,gluon,qbt,qbtbr,gluon,       &
                      qtop,qtopbr,gluon,                                   &
                      qupl,qupbrl,photon, qupr,qupbrr,photon, &
                      qdr,qdbrr,photon, qdl,qdbrl,photon,  &
                      qcr,qcbrr,photon, qcl,qcbrl,photon, &
                      qsr,qsbrr,photon, qsl,qsbrl,photon,      &
                      qbt,qbtbr,photon, qtop,qtopbr,photon,  &
                      gluon,gluon,gluon /)       ! the first entry is
...
!
  chdim : select case (npr)
    case (4)
      dim(d_offsh)=60; dim(d_offshcol)=40; dim(d_ampfld)=30; dim(d_ampcol)=20
    case (5)
      dim(d_offsh)=90; dim(d_offshcol)=50; dim(d_ampfld)=50; dim(d_ampcol)=30
    case (8)
      dim(d_offsh)=710; dim(d_offshcol)=420; dim(d_ampfld)=460; dim(d_ampcol)=230
    case (7)
      dim(d_offsh)=360; dim(d_offshcol)=220; dim(d_ampfld)=230; dim(d_ampcol)=120
    case (6)
      dim(d_offsh)=190; dim(d_offshcol)=110; dim(d_ampfld)=130; dim(d_ampcol)=70
    case (10)
      dim(d_offsh)=2740; dim(d_offshcol)=1620; dim(d_ampfld)=1780; dim(d_ampcol)=890
    case (9)
      dim(d_offsh)=1390; dim(d_offshcol)=820; dim(d_ampfld)=910; dim(d_ampcol)=460
    case default
