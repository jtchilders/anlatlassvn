...
#if USE_RNG_MERSENNE_TWISTER

C-----------------------------------------------------------------------
      FUNCTION RANGEN(I)
C-----------------------------------------------------------------------
C     MAIN RANDOM NUMBER GENERATOR
C     USES MERSENNE TWISTER IMPL 
C-----------------------------------------------------------------------
      IMPLICIT NONE
      DOUBLE PRECISION RANGEN,RANSET,RANGET
      INTEGER I,ISEED(3),JSEED(2)
      integer mpirank 
      character*5 rankstring
      common/mpi/mpirank,rankstring
      double precision genrand_real3
      SAVE ISEED
      RANGEN=genrand_real3();
      RETURN
C-----------------------------------------------------------------------
      ENTRY RANSET(JSEED)
C-----------------------------------------------------------------------
...
c  For MPI
     ...
     $        ,tmpstr(1:l)
c         call alustc(fname,'.wgt',tmpstr)
