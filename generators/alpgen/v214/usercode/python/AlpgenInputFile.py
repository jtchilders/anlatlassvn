...
class AlpgenInputFile:
   ...
   def __str__(self):
      ...
      for option_name in sorted(self.options.keys()):
         txt += self.options[option_name].get_line() + '\n'
   ...
   def edit_option(self,label,value):
      self.options[label].value = value
      ...
      for line in file:
         ...
         try:
            if counter == 0: #  imode
               ...
               if len(strings) >= 2:
                  option = AlpgenOption.parse_line(line)
                  self.options[option.label] = option
                  continue
         except IndexError,e:
            logger.exception('strings: '  + str(strings))
            raise
