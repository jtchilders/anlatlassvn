...
private:
  ...
  static std::string trim(string s);
...
private:
  ...
  // Class variables
  std::string    baseFN, parFN, unwFN;  // Incoming filenames
  ...
  std::ifstream  ifsUnw;                // Input file stream for 'unw' file
  std::istream*  isUnw;                 // Input stream for 'unw' file
  std::vector<Pythia8::LHAParticle> myParticles; // Local storage for particles
...
public:
  ...
  bool parse(const std::string paramStr);

  // Parse an incoming parameter line
  void extractRunParam(std::string line);

  // Check if a parameter exists
  bool haveParam(const std::string &paramIn) {
    return (params.find(paramIn) == params.end()) ? false : true; }

  // Get a parameter as a double or integer.
  // Caller should have already checked existance of the parameter.
  double getParam(const std::string &paramIn) {
    return (haveParam(paramIn)) ? params[paramIn] : 0.; }
  int    getParamAsInt(const std::string &paramIn) {
    return (haveParam(paramIn)) ? int(params[paramIn]) : 0.; }
...
private:
  ...
  // Warn if a parameter is going to be overwriten
  void warnParamOverwrite(const std::string &paramIn, double val);
...
bool AlpgenPar::parse(const std::string paramStr) {
...
void AlpgenPar::extractRunParam(std::string line) {

  // Extract information to the right of the final '!' character
  size_t idx = line.rfind("!");
  if (idx == string::npos) return;
  std::string paramName = trim(line.substr(idx + 1));
  std::string paramVal  = trim(line.substr(0, idx));
  std::istringstream iss(paramVal);
  ...
  // Special case: 'mc,mb,...' - split on ',' for name and ' ' for values
    ...
    // Simple tokeniser
    std::string        paramNameNow;
    std::istringstream issName(paramName);
...
void AlpgenPar::printParams() {
  ...
  std::cout << fixed << setprecision(3) << endl
       << " *-------  Alpgen parameters  -------*" << std::endl;
  for (map < string, double >::iterator it = params.begin();
       it != params.end(); ++it)
    std::cout << " |  " << left << setw(13) << it->first
...
void AlpgenPar::warnParamOverwrite(const std::string &paramIn, double val) {
...
std::string AlpgenPar::trim(std::string s) {
...
LHAupAlpgen::LHAupAlpgen(const char* baseFNin, Pythia8::Info* infoPtrIn)
  ...
  if (infoPtr){string key="AlpgenPar"; setInfoHeader(key, paramStr);}
...
bool LHAupAlpgen::setEvent(int, double) {
  ...
  for (int i = 0; i < nParton; i++) {
    ...
    // Add particle
    myParticles.push_back(Pythia8::LHAParticle(
...
void LHAupAlpgen::printParticles() {
  ...
  std::cout << std::endl << "---- LHAupAlpgen particle listing begin ----" << std::endl;
  ...
  std::cout << "----  LHAupAlpgen particle listing end  ----" << std::endl;
}
...
bool LHAupAlpgen::addResonances() {
  ...
  if (lprup == 13) for (int i = 0; i < 2; i++)
    if (abs(myParticles[i].idPart) == 5) {
      ...
      myParticles[i].ePart = sqrt(Pythia8::pow2(myParticles[i].pzPart) + Pythia8::pow2(mb));
    }
...
bool LHAupAlpgen::rescaleMomenta() {
  ...
  int  nOut = 0;
  Pythia8::Vec4 pIn, pOut;
  for (int i = 0; i < int(myParticles.size()); i++) {
    Pythia8::Vec4 pNow = Pythia8::Vec4(myParticles[i].pxPart, myParticles[i].pyPart,
  ...
  if (abs(pOut.pT() - pIn.pT()) > ZEROTHRESHOLD) {
    ...
    if (pxDiff > PTWARNTHRESHOLD || pyDiff > PTWARNTHRESHOLD) {
      ...
      if (LHADEBUGRESCALE) {
        printParticles();
        std::cout << "pxDiff = " << pxDiff << ", pyDiff = " << pyDiff << std::endl;
      }
    }
    ...
    for (int i = 2; i < int(myParticles.size()); i++) {
      ...
      myParticles[i].ePart   = sqrt(max(0., Pythia8::pow2(myParticles[i].pxPart) +
          Pythia8::pow2(myParticles[i].pyPart) + Pythia8::pow2(myParticles[i].pzPart) +
          Pythia8::pow2(myParticles[i].mPart)));
      pOut += Pythia8::Vec4(myParticles[i].pxPart, myParticles[i].pyPart,
                   myParticles[i].pzPart, myParticles[i].ePart);
    }
  }
  ...
  if (abs(a - 1.) * myParticles[0].ePart > EWARNTHRESHOLD ||
    ...
    if (LHADEBUGRESCALE) {
      printParticles();
      std::cout << "de = " << de << ", dp = " << dp
           << ", a = " << a << ", b = " << b << endl
           << "Absolute energy change for incoming 0 = "
           << abs(a - 1.) * myParticles[0].ePart << std::endl
           << "Absolute energy change for incoming 1 = "
           << abs(b - 1.) * myParticles[1].ePart << std::endl;
    }
  }
  ...
  for (int i = 0; i < int(myParticles.size()); i++) {
    ...
    for (int j = 0; j < int(myParticles.size()); j++) {
      ...
      resVec += Pythia8::Vec4(myParticles[j].pxPart, myParticles[j].pyPart,
...
AlpgenHooks::AlpgenHooks(Pythia8::Pythia &pythia) : LHAagPtr(NULL) {

  // If LHAupAlpgen needed, construct and pass to Pythia
  std::string agFile = pythia.settings.word("Alpgen:file");
  if (agFile != "void") {
...
bool AlpgenHooks::initAfterBeams() {
  ...
  std::string parStr = infoPtr->header("AlpgenPar");
  if (!parStr.empty()) {
    par.parse(parStr);
    par.printParams();
  }
...
void MadgraphPar::extractRunParam(string line) {
  ...
  if (idx == string::npos) return;
  std::string paramName = trim( line.substr( idx + 1, idy - idx - 1) );
  std::string paramVal  = trim( line.substr( 0, idx) );
...
void MadgraphPar::printParams() {
  ...
  // Loop over all stored parameters and print
  std::cout << std::endl
       << " *--------  Madgraph parameters  --------*" << std::endl;
  for (map<string,double>::iterator it = params.begin();
       it != params.end(); ++it)
    std::cout << " |  " << left << setw(15) << it->first
